...
(() => {
  ...
  function filteredRows(){const card=ensure(),q=String(card?.querySelector('#cxAdminSealedSearch')?.value||'').trim().toLowerCase(),subtype=String(card?.querySelector('#cxAdminSealedType')?.value||''),mapping=String(card?.querySelector('#cxAdminSealedMapping')?.value||'');return products.filter(r=>{const hay=`${r.name||''} ${r.set_code||''} ${setNames.get(String(r.set_code||'').toUpperCase())||''}`.toLowerCase();if(q&&!hay.includes(q))return false;if(subtype&&r.subtype!==subtype)return false;if(mapping==='mapped'&&!r.tcgplayer_product_id)return false;if(mapping==='unmapped'&&r.tcgplayer_product_id)return false;return true})}
  ...
  function productHtml(r,enabled,pm,em){
    const pp=pm.get(String(r.uuid))||[],tcg=pp.find(x=>x.source==='tcgplayer_public'),ck=pp.find(x=>x.source==='cardkingdom_public'),mapped=Boolean(r.tcgplayer_product_id),priceTracked=Boolean(tcg||ck),px=tcg?.low_price??tcg?.market_price??ck?.market_price??null,ev=em.get(String(r.uuid));
    const tags=[`<span class="cx-admin-sealed-tag">${esc(r.category||'unknown')}</span>`,`<span class="cx-admin-sealed-tag">${esc(r.subtype||'—')}</span>`,mapped?'<span class="cx-admin-sealed-tag good">Exact TCG ID</span>':'<span class="cx-admin-sealed-tag warn">Needs exact TCG ID</span>',lifecycleTag(ev,mapped,priceTracked),ev?.scout_sealed_grade?`<span class="cx-admin-sealed-tag good">${esc(ev.scout_sealed_grade)} · ${esc(ev.scout_sealed_score)}</span>`:''].filter(Boolean).join('');
