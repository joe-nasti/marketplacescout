...
(() => {
  ...
  function ensure(){
    ...
    card.innerHTML=`<div class="cx-admin-catalog-head"><div><div class="cx-section-title">Sealed product catalog</div><p>Browse MTGJSON sealed identity separately from what Scout Sealed currently scores.</p></div><span id="cxAdminSealedCount" class="cx-sub"></span></div>
      <div class="cx-admin-sealed-tools"><input id="cxAdminSealedSearch" placeholder="Search sealed products… e.g. Secret Lair"><select id="cxAdminSealedCategory"><option value="">All categories</option><option value="box_set">Box set</option><option value="deck">Deck</option><option value="bundle">Bundle</option><option value="bundle_case">Bundle case</option></select><select id="cxAdminSealedMapping"><option value="">All identity</option><option value="mapped">TCG mapped</option><option value="unmapped">Needs TCG match</option></select></div>
    ...
    card.querySelector('#cxAdminSealedCategory').addEventListener('change',()=>load(true));
  ...
  async function load(force=false){
    ...
    const category=String(card.querySelector('#cxAdminSealedCategory')?.value||'');
    const mapping=String(card.querySelector('#cxAdminSealedMapping')?.value||'');
    const key=`${q}|${category}|${mapping}`;if(!force&&key===lastKey)return;lastKey=key;loading=true;
    ...
    try{
      ...
      out.innerHTML=rows.map(r=>{
        const pp=pm.get(String(r.uuid))||[],tcg=pp.find(x=>x.source==='tcgplayer'),ck=pp.find(x=>x.source==='cardkingdom');
