...
(() => {
  ...
  let products=[],prices=[],evRows=[],pipelineState=null,setNames=new Map(),setTypes=new Map(),enabledSets=new Set(),expanded=new Set();
  ...
  function ensure(){
    ...
    card.innerHTML=`<div class="cx-admin-catalog-head"><div><div class="cx-section-title">Sealed set management</div><p>Enable a set to process every sealed product in it through exact identity, pricing, contents, component EV and Scout Sealed scoring.</p><div id="cxAdminSealedPipeline" class="cx-sub"></div></div><span id="cxAdminSealedCount" class="cx-sub"></span></div>
      <div class="cx-admin-sealed-tools">
        ...
        <select id="cxAdminSealedType"><option value="">All MTGJSON set types</option></select>
        <select id="cxAdminSealedPackage"><option value="">All product packaging</option></select>
        ...
        <select id="cxAdminSealedPrice"><option value="">All sealed pricing</option><option value="priced">Price tracked</option><option value="unpriced">Waiting on price</option></select>
        <select id="cxAdminSealedEv"><option value="">All EV states</option><option value="ready">EV ready</option><option value="not_ready">EV not ready</option></select>
        <select id="cxAdminSealedGrade"><option value="">All Scout grades</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option><option value="F">F</option><option value="ungraded">Ungraded</option></select>
        <select id="cxAdminSealedBlocker"><option value="">All blockers</option><option value="none">No blocker</option><option value="tcg_exact_id_missing">Missing exact TCG ID</option><option value="sealed_price_missing">Missing sealed price</option><option value="no_card_components_after_expansion">No card components</option><option value="market_coverage_below_50pct">Market coverage below 50%</option></select>
        <select id="cxAdminSealedRelease"><option value="">All release dates</option><option value="future">Future releases</option><option value="90d">Last 90 days</option><option value="1y">Last year</option><option value="older">Older than 1 year</option></select>
      </div>
    ...
    card.querySelectorAll('select').forEach(x=>x.addEventListener('change',render));
  ...
  function effectiveLifecycle(r,ev,pm){const mapped=Boolean(r.tcgplayer_product_id),priced=Boolean((pm.get(String(r.uuid))||[]).length);return ev?.lifecycle_status||(mapped?(priced?'price_tracked':'tcg_mapped'):'catalog_only')}
  function populateTaxonomyFilters(){
    const card=ensure();if(!card)return;
    const type=card.querySelector('#cxAdminSealedType'),pkg=card.querySelector('#cxAdminSealedPackage');
    const keepType=type.value,keepPkg=pkg.value;
    const types=[...new Set([...setTypes.values()].filter(Boolean))].sort();
    const packages=[...new Set(products.map(x=>x.subtype).filter(Boolean))].sort();
    type.innerHTML='<option value="">All MTGJSON set types</option>'+types.map(x=>`<option value="${esc(x)}">${esc(human(x))}</option>`).join('');
    pkg.innerHTML='<option value="">All product packaging</option>'+packages.map(x=>`<option value="${esc(x)}">${esc(human(x))}</option>`).join('');
    if(types.includes(keepType))type.value=keepType;if(packages.includes(keepPkg))pkg.value=keepPkg;
  }
  function filteredRows(){
    const card=ensure(),pm=priceMap(),em=evMap(),q=String(card?.querySelector('#cxAdminSealedSearch')?.value||'').trim().toLowerCase();
    const setType=String(card?.querySelector('#cxAdminSealedType')?.value||''),pkg=String(card?.querySelector('#cxAdminSealedPackage')?.value||''),mapping=String(card?.querySelector('#cxAdminSealedMapping')?.value||''),life=String(card?.querySelector('#cxAdminSealedLifecycle')?.value||''),price=String(card?.querySelector('#cxAdminSealedPrice')?.value||''),evState=String(card?.querySelector('#cxAdminSealedEv')?.value||''),grade=String(card?.querySelector('#cxAdminSealedGrade')?.value||''),blocker=String(card?.querySelector('#cxAdminSealedBlocker')?.value||''),release=String(card?.querySelector('#cxAdminSealedRelease')?.value||'');
    const now=Date.now(),d90=90*864e5,d1y=365*864e5;
    return products.filter(r=>{
      const code=String(r.set_code||'').toUpperCase(),ev=em.get(String(r.uuid)),tracked=(pm.get(String(r.uuid))||[]).length>0,stage=effectiveLifecycle(r,ev,pm),d=r.release_date?new Date(r.release_date+'T00:00:00').getTime():0;
      const hay=`${r.name||''} ${code} ${setNames.get(code)||''} ${setTypes.get(code)||''}`.toLowerCase();
      if(q&&!hay.includes(q))return false;if(setType&&setTypes.get(code)!==setType)return false;if(pkg&&r.subtype!==pkg)return false;
      if(mapping==='mapped'&&!r.tcgplayer_product_id)return false;if(mapping==='unmapped'&&r.tcgplayer_product_id)return false;if(life&&stage!==life)return false;
      if(price==='priced'&&!tracked)return false;if(price==='unpriced'&&tracked)return false;
      const evReady=['ev_ready','scout_sealed'].includes(stage);if(evState==='ready'&&!evReady)return false;if(evState==='not_ready'&&evReady)return false;
      if(grade==='ungraded'&&ev?.scout_sealed_grade)return false;if(grade&&grade!=='ungraded'&&ev?.scout_sealed_grade!==grade)return false;
      if(blocker==='none'&&ev?.blocker)return false;if(blocker&&blocker!=='none'&&ev?.blocker!==blocker)return false;
      if(release==='future'&&!(d>now))return false;if(release==='90d'&&!(d<=now&&d>=now-d90))return false;if(release==='1y'&&!(d<=now&&d>=now-d1y))return false;if(release==='older'&&!(d&&d<now-d1y))return false;
      return true;
    });
  }
  ...
  function productHtml(r,enabled,pm,em){
    const pp=pm.get(String(r.uuid))||[],tcg=pp.find(x=>x.source==='tcgplayer_public'),ck=pp.find(x=>x.source==='cardkingdom_public'),mapped=Boolean(r.tcgplayer_product_id),priceTracked=Boolean(tcg||ck),px=tcg?.low_price??tcg?.market_price??ck?.market_price??null,ev=em.get(String(r.uuid)),code=String(r.set_code||'').toUpperCase();
    const tags=[setTypes.get(code)?`<span class="cx-admin-sealed-tag">${esc(human(setTypes.get(code)))}</span>`:'',`<span class="cx-admin-sealed-tag">${esc(r.category||'unknown')}</span>`,`<span class="cx-admin-sealed-tag">${esc(r.subtype||'—')}</span>`,mapped?'<span class="cx-admin-sealed-tag good">Exact TCG ID</span>':'<span class="cx-admin-sealed-tag warn">Needs exact TCG ID</span>',lifecycleTag(ev,mapped,priceTracked),ev?.scout_sealed_grade?`<span class="cx-admin-sealed-tag good">${esc(ev.scout_sealed_grade)} · ${esc(ev.scout_sealed_score)}</span>`:''].filter(Boolean).join('');
  ...
  async function load(force=false){
    ...
    ]);products=p||[];prices=px||[];evRows=ev||[];pipelineState=(state||[])[0]||null;setNames=new Map((catalog||[]).map(x=>[String(x.code||'').toUpperCase(),x.name]));setTypes=new Map((catalog||[]).map(x=>[String(x.code||'').toUpperCase(),x.set_type]));enabledSets=new Set((profiles||[]).filter(x=>x.enabled).map(x=>String(x.set_code||'').toUpperCase()));loaded=true;populateTaxonomyFilters();render()}catch(e){if(out)out.innerHTML=`<div class="cx-admin-error">Couldn’t load sealed sets: ${esc(e.message||e)}</div>`}finally{loading=false}
  }
