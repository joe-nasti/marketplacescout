...
async function queueProbe(userId,kind,url,extra={}){if(await active(userId,kind))return false;const priority=kind==='last_updated'?1:2;await sb('collector_jobs',{method:'POST',body:[{user_id:userId,source:'agent',action:'seller_portal_readonly_probe',status:'queued',priority,required_capability:'tcgplayer_authenticated_session',preferred_executor:'android_agent',payload_json:{sypKind:kind,...extra,probe:{mode:'fetch_text',method:'GET',url}},progress_json:{stage:'queued',percent:0,detail:`SYP ${kind} read-only probe queued`,updatedAt:nowIso()},max_attempts:3}],prefer:'return=minimal'});return true}
