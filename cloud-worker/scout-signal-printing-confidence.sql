-- Apply printing specificity to MTGStocks Interests inside Scout's priority overlay.
-- Exact-printing movement gets full context. Oracle-family inheritance is
-- attenuated unless multiple exact printings corroborate the same move.
...
with recent as (
  select distinct
    l.user_id,l.product_id,l.oracle_id,l.canonical_name,l.intel_id,l.family_match,
    ...
    case
      when i.source_name='MTGStocks' and i.source_subtype='interests' and not l.family_match
        and ipc.source_product_id=l.product_id then 1.00
    ...
    case when i.source_name='MTGStocks' and i.source_subtype='interests' then ipc.printing_scope end interest_printing_scope,
  ...
  select user_id,product_id,
    ...
    count(*) filter(where family_match)::int inherited_signal_count,count(*) filter(where not family_match)::int exact_signal_count,
...
select user_id,product_id,oracle_id,canonical_name,signal_count,independent_sources,leading_sources,confirming_sources,
  ...
  case when weighted_net<=0 then 'Recent Signals are mixed or bearish; no Scout priority boost.'
       when interest_inherited_signal_count>0 and interest_exact_signal_count=0 and interest_corroborating_printings<2 then 'MTGStocks Interests movement is isolated to another printing; it is weak Oracle-family context and does not change the Scout grade.'
