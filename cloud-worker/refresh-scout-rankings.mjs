...
try{
  ...
  const totalMs=Object.values(durations).reduce((a,b)=>a+b,0);
  const detail={aggregate,shadow,cache,refresh_reason:decision.reason,watermarks:decision.watermarks,durations_ms:durations,total_ms:totalMs,model:'watermark gate -> 24h -> v5 shadow -> promoted cache'};
  ...
  const totalMs=Object.values(durations).reduce((a,b)=>a+b,0);
  await state('failed',{failed_phase:phase,error:e.message,refresh_reason:decision.reason,watermarks:decision.watermarks,durations_ms:durations,total_ms:totalMs},started).catch(()=>{});
