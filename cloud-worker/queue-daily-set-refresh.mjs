...
// create exactly one browser fallback. Without this guard, the queue step could create
// a second same-set job before recovery sees the failed one.
const existing=await allRows('collector_jobs?select=user_id,status,payload_json,created_at&source=eq.marketplace&action=eq.scan_set&status=in.(queued,claimed,running,failed)');
...
for(const j of existing){
  ...
  if(j.status==='failed'&&isDaily&&j.created_at&&chicagoDay(j.created_at)===today)coveredByJob.add(key);
}
...
console.log(`Daily set refresh ${today} America/Chicago: ${latest.size} historical user/set pairs, ${completedToday.size} already scanned today, ${coveredByJob.size} covered by queued/running/today-failed jobs, ${jobs.length} newly queued.`);
