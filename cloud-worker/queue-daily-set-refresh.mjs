...
const jobs=[];
for(const [key,s] of latest){if(completedToday.has(key)||coveredByJob.has(key))continue;const p=s.profile_json||{};jobs.push({user_id:s.user_id,source:'marketplace',action:'scan_set',status:'queued',priority:30,required_capability:'marketplace_public_api',preferred_executor:'cloud_worker',payload_json:{profile:{setName:s.set_name,setSlug:s.set_slug,language:s.language||'English',printing:s.printing||'Both',condition:s.condition||'Near Mint',scanDepth:p.scanDepthRequested||p.scanDepth||'Smart',salesEnrich:0},cloudPrimary:true,dailyAutoSync:true,executionClass:'cloud_public',sourceCapturedAt:s.captured_at,salesStrategy:'scout-prioritized-cache-v1'},progress_json:{stage:'queued',percent:0,detail:'Daily one-scan-per-set refresh; sales history is enriched centrally by Scout priority',updatedAt:new Date().toISOString()},attempt_count:0,max_attempts:2,available_at:new Date().toISOString()})}
if(jobs.length)await sb('collector_jobs',{method:'POST',body:jobs,prefer:'return=minimal'});
console.log(`Daily set refresh ${today} America/Chicago: ${latest.size} historical user/set pairs, ${completedToday.size} completed today, ${coveredByJob.size} covered by queued/running/today-failed/recent-catchup jobs, ${jobs.length} newly queued. Sales history is handled by the prioritized Scout cache.`);
