...
await sb('mtgjson_sync_state?on_conflict=feed',{method:'POST',body:[{feed:'cardtrader_web_links',status:'complete',last_started_at:new Date().toISOString(),last_completed_at:new Date().toISOString(),row_count:rows.length,detail:{version:'ct-web-link-v1',expansions:expansions.length,blueprints:rows.length,with_url:rows.filter(x=>x.raw_json.web_url).length}}],prefer:'resolution=merge-duplicates,return=minimal'});
