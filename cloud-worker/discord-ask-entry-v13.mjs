import entry from './discord-ask-entry-v12.mjs';
...
function reasonBits(row) {
  const bits = [];
  if (Number(row.signal_count || 0) > 0) bits.push(`${row.signal_count} Signals/${row.independent_sources || 0} sources`);
  if (Number(row.competitive_decks_7d || 0) > Number(row.competitive_decks_prev_7d || 0)) bits.push(`${row.competitive_decks_7d} comp decks 7d vs ${row.competitive_decks_prev_7d || 0}`);
...
function specPayload(candidates) {
  ...
  return {
    ...
    embeds: [{
      ...
      fields,
      footer: { text: 'Collectish public-market spec ranking • refine naturally with a short follow-up /ask' },
    }],
    components: [],
  };
}
...
function conversationText(candidates) {
  const lines = candidates.slice(0, 10).map((c) => `${c.rank}. ${c.card_name} (${c.set_code} ${c.printing || ''}) — Market ${money(c.market_price)}, spec ${c.spec_score}; ${reasonBits(c).join(', ') || `Scout ${c.scout_grade} ${c.scout_score}`}.`);
...
async function ensureGuestSession(env, job, binding) {
  ...
  const response = await fetch(`${supabaseBase(env)}/functions/v1/ask-collectish-api`, {
    ...
    body: JSON.stringify({
      action: 'session.create', client: 'discord_guest', guest: true,
...
async function persistTurnBestEffort(env, job, candidates) {
  try {
    ...
    const owners = await serviceRest(env, `discord_guest_auth_sessions?discord_user_id=eq.${encodeURIComponent(String(job.discord_user_id || ''))}&select=anonymous_user_id&limit=1`).catch(() => []);
    const owner = owners?.[0]?.anonymous_user_id || null;
    if (!owner || !sessionId) return;
    const now = new Date().toISOString();
    const text = conversationText(candidates);
    await serviceRest(env, 'ask_collectish_messages', {
      method: 'POST', prefer: 'return=minimal', body: [
        { user_id: owner, conversation_id: sessionId, role: 'user', content: String(job.question || ''), metadata: { screen: 'discord', route: 'public_spec_candidates' }, created_at: now },
        { user_id: owner, conversation_id: sessionId, role: 'assistant', content: text, metadata: { route: 'public_spec_candidates', candidate_count: candidates.length }, created_at: now },
      ],
    });
    await serviceRest(env, `ask_collectish_conversations?id=eq.${encodeURIComponent(sessionId)}&user_id=eq.${encodeURIComponent(owner)}`, { method: 'PATCH', body: { updated_at: now } });
  } catch (error) {
    console.error('discord ask v13 spec persistence failed', { interaction_id: job.interaction_id, error: String(error?.message || error) });
  }
}

async function handleSpec(env, job, message, ctx) {
  ...
  try {
    ...
    // Delivery is the critical path. Session persistence must never block the answer.
    await editOriginalDiscord(job, specPayload(candidates));
    const text = conversationText(candidates);
    await updateDelivery(env, job.interaction_id, {
      response_text: text.slice(0, 1900), status: 'completed', completed_at: new Date().toISOString(), error_text: null,
    });
    message.ack();
    ctx.waitUntil(persistTurnBestEffort(env, job, candidates));
  } catch (error) {
    const detail = String(error?.message || error).slice(0, 500);
    await editOriginalDiscord(job, { content: `Delvin couldn't finish the spec ranking: ${detail}`, embeds: [], components: [] }).catch(() => null);
...
async function handleQueue(batch, env, ctx) {
  ...
  for (const message of batch.messages) {
    ...
    await handleSpec(env, job, message, ctx);
  }
