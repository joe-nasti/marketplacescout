-- Direct realization-aware Scout scoring.
--
-- High TCG Direct asks are attractive only when the exact SKU has evidence that
-- the premium can actually be realized. For Direct asks >= 1.75x TCG Market,
-- discount the Direct-execution portion of V5 using:
--   * the highest observed exact-SKU sale-with-shipping in the latest 90-day history,
--   * exact-SKU marketplace velocity,
--   * exact-SKU SYP eligibility as a restock-risk penalty.
--
-- This leaves structural thesis and buylist backing intact. A thin SKU with a
-- strong cash floor can still be attractive, but an unproven Direct ask cannot
-- by itself manufacture an A grade.
...
with s as (
  ...
  select x.*,
    ...
    least(70.0,opportunity_score*70.0/85.0) structural_points,
    ...
    coalesce((
      select max(nullif((b->>'highSalePriceWithShipping')::numeric,0))
      from jsonb_array_elements(coalesce(sales_raw->'buckets','[]'::jsonb)) b
    ),0) historical_high_sale_ship
  from x
), q as (
  select a.*,
    ...
    case
      when direct_low<=0 or sku_market_price<=0 or direct_low < sku_market_price*1.75 then 1::numeric
        ...
        least(
          case
            when historical_high_sale_ship>0 then least(1::numeric,historical_high_sale_ship/(direct_low*.75))
            else .15::numeric
          end,
          case
        ...
        ) * case when syp_current then .65::numeric else 1::numeric end
      )
  ...
  select user_id,sku_id,product_id,product_name,set_name,set_code,collector_number,printing,uuid,now(),opportunity_score,v5_score,
    ...
    case
      when direct_low>=sku_market_price*1.75 and direct_realization_factor<.70 and syp_current then 'direct_gap_restock_risk'
      when direct_low>=sku_market_price*1.75 and direct_realization_factor<.70 then 'speculative_direct_gap'
      when uuid is null then 'identity_missing'
      when source_verify then 'verify_source'
      when buylist_backed then 'buylist_backed'
      when confirmation_points>=3.5 then 'market_confirmed'
      else 'market_mixed'
    end,
    jsonb_build_object(
      'version','v5-shadow-4',
      'model','structure70+realization-adjusted-execution20+liquidity5+confirmation5',
      'tcgMarketRole','index',
      'tcgLowRole','retail acquisition',
      'tcgDirectLowRole','direct benchmark / modeled exit',
      'structural',round(structural_points,2),
      'directExecution',round(direct_execution_points,2),
      'directExecutionRaw',round(direct_execution_raw*execution_confidence_factor,2),
      'directRealizationFactor',round(direct_realization_factor,3),
      'historicalHighSaleWithShipping',historical_high_sale_ship,
      'exactSkuSalesPerDay',avg_daily_qty_sold,
      'directInventoryAvailable',direct_available,
      'sypCurrentlyEligible',syp_current,
      'buylistBacking',round(buylist_backing_points,2),
      'liquidity',round(liquidity_points,2),
      'confirmation',round(confirmation_points,2),
      'executionConfidenceFactor',execution_confidence_factor,
      'cheapestSource',cheapest_source,
      'cheapestBuy',cheapest_buy
