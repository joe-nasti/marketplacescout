...
const MATCH=(process.env.SIGNALS_RETAG_MATCH||'').split('|').map(x=>x.trim()).filter(Boolean);
...
async function tag(g){const r=await fetch(`${U}/functions/v1/market-intel-card-tag`,{method:'POST',headers:H,body:JSON.stringify({source_url:g.url,rendered_title:g.title,intel_ids:g.ids})});const t=await r.text();let d;try{d=t?JSON.parse(t):{}}catch{d={error:t}};if(!r.ok)throw new Error(d?.error||`Tagger ${r.status}`);return d}
async function tagWithRetry(g){let last;for(let attempt=1;attempt<=2;attempt++){try{return await tag(g)}catch(e){last=e;if(attempt<2)await new Promise(r=>setTimeout(r,1200))}}throw last}
...
const queue=[...groups.values()].filter(g=>!MATCH.length||MATCH.some(x=>g.url.includes(x))).slice(0,LIMIT);let cursor=0,checked=0,tagged=0,resolved=0,nonMagic=0,failed=0;const failures=[];
async function worker(){while(true){const ix=cursor++;if(ix>=queue.length)return;const g=queue[ix];checked++;try{const d=await tagWithRetry(g);tagged+=Number(d?.tagged||0);resolved+=Number(d?.cards?.length||0);if(d?.is_magic===false)nonMagic++;console.log(JSON.stringify({source:g.url,cards:Number(d?.cards?.length||0),tagged:Number(d?.tagged||0),is_magic:d?.is_magic!==false,method:d?.method||null,list_candidates:Number(d?.list_candidates||0)}))}catch(e){failed++;failures.push({source:g.url,error:e?.message||String(e)});console.error(JSON.stringify(failures.at(-1)))}}}
await Promise.all(Array.from({length:Math.min(CONCURRENCY,queue.length||1)},()=>worker()));
const summary={ok:failed===0,days:DAYS,groups:groups.size,selected:queue.length,match:MATCH,checked,tagged,resolved,non_magic:nonMagic,failed,failures};console.log(JSON.stringify(summary));
