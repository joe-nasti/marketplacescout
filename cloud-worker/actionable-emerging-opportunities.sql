...
with comp as (
  ...
  select lower(card_name) k, product_id, sku_id,
         ...
         least(100,greatest(0,70+coalesce(round(rank_improvement_pct/4.0),0)))::int strength,
         watch_reason reason
  from public.commander_edh_opportunities(200)
  where watch_class='edh_breakout'
), cedh as (
  select lower(card_name) k, product_id, sku_id,
         'cEDH'::text family,
  ...
  from liquid l
  join agg a on a.sku_id=l.sku_id
)
