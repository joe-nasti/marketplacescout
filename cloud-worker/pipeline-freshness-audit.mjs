...
const SALES_STALE_MINUTES=Math.max(180,Number(process.env.SALES_STALE_MINUTES||300));
const VOLATILITY_STALE_MINUTES=Math.max(360,Number(process.env.VOLATILITY_STALE_MINUTES||540));
...
const normalize=(row,staleAfter,cadence)=>{const at=row?.last_completed_at||null;const age=ageMinutes(at);return{status:row?.status||null,last_completed_at:at,age_minutes:Number.isFinite(age)?Math.round(age):null,stale_after_minutes:staleAfter,cadence,detail:row?.detail||{}}};
try{
  const [priceRows,scoreRows,stateRows]=await Promise.all([
    ...
    sb('mtgjson_sync_state?select=feed,status,last_completed_at,detail&feed=in.(marketplace_sales_history,scout_volatility)&limit=10')
  ]);
  ...
  const sales=normalize((stateRows||[]).find(x=>x.feed==='marketplace_sales_history'),SALES_STALE_MINUTES,'every 3h');
  const volatility=normalize((stateRows||[]).find(x=>x.feed==='scout_volatility'),VOLATILITY_STALE_MINUTES,'every 6h');
  const problems=[];
  ...
  if(!sales.last_completed_at)problems.push('sales_missing_watermark');
  else if(sales.status==='failed'||sales.age_minutes>SALES_STALE_MINUTES)problems.push('sales_pipeline_stale');
  if(!volatility.last_completed_at)problems.push('volatility_missing_watermark');
  else if(volatility.status==='failed'||volatility.status==='paused'||volatility.age_minutes>VOLATILITY_STALE_MINUTES)problems.push('volatility_pipeline_stale');
  const detail={checked_at:new Date().toISOString(),tcgplayer:{source:'TCGCSV',cadence:'daily',source_updated_at:priceAt,age_minutes:Number.isFinite(priceAge)?Math.round(priceAge):null,stale_after_minutes:TCGCSV_STALE_MINUTES,status:price.status||null},scout_score:{v5_computed_at:scoreAt,age_minutes:Number.isFinite(scoreAge)?Math.round(scoreAge):null,note:'Recovery owned by watch-scout-rankings.mjs in independent alerts workflow.'},sales,volatility,problems};
