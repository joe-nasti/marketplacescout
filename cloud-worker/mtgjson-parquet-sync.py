...
def idget(x,*keys):
    lowered={str(k).lower():v for k,v in (x or {}).items()}
    for k in keys:
...
def sync_cards():
    ...
    for c in cards:
        ...
        out.append({'uuid':uid,'name':str(pick(c,'name',default='') or '').strip() or '(unknown)',
          ...
          'finishes':arr(pick(c,'finishes')),'availability':arr(pick(c,'availability')),
          'scryfall_id':idget(x,'scryfallId') if is_uuid(idget(x,'scryfallId')) else None,
          'scryfall_oracle_id':idget(x,'scryfallOracleId') if is_uuid(idget(x,'scryfallOracleId')) else None,
          'tcgplayer_product_id':text(idget(x,'tcgplayerProductId')),'tcgplayer_etched_product_id':text(idget(x,'tcgplayerEtchedProductId')),
          'tcgplayer_alt_foil_product_id':text(idget(x,'tcgplayerAlternativeFoilProductId')),
          'cardkingdom_id':text(idget(x,'cardKingdomId')),'cardkingdom_foil_id':text(idget(x,'cardKingdomFoilId')),
...
def sync_sealed():
    ...
    for p in read_rows('sealedProducts'):
        ...
        out.append({'uuid':uid,'set_code':text(pick(p,'setCode','set_code','code')),'name':str(pick(p,'name',default='') or '').strip() or '(unknown)',
          'category':text(pick(p,'category')),'subtype':text(pick(p,'subtype')),'release_date':datev(pick(p,'releaseDate','release_date')),
          'card_count':pick(p,'cardCount','card_count'),'product_size':pick(p,'productSize','product_size'),
          'tcgplayer_product_id':text(idget(ids,'tcgplayerProductId') or pick(p,'tcgplayerProductId')),
