...
async function fetchExistingWindow(userId,from,to){
  if(!from||!to)return [];
  return await sb(
    `seller_orders?select=order_number,order_date,order_status,order_channel,order_fulfillment,buyer_name,shipping_type,has_details&user_id=eq.${enc(userId)}&order_date=gte.${enc(from)}&order_date=lt.${enc(to)}&limit=5000`
  );
}
function summaryNeedsDetail(raw,normalized,existing){
  if(!existing||!existing.has_details)return true;
  const checks=[
    [raw?.orderStatus??raw?.status,existing.order_status],
    [raw?.orderChannel,existing.order_channel],
    [raw?.orderFulfillment,existing.order_fulfillment],
    [raw?.buyerName,existing.buyer_name],
    [raw?.shippingType,existing.shipping_type]
  ];
  for(const [incoming,current] of checks){
    if(incoming!=null && String(incoming)!==String(current??''))return true;
  }
  if(normalized?.order_date && existing.order_date){
    const a=new Date(normalized.order_date).getTime(),b=new Date(existing.order_date).getTime();
    if(Number.isFinite(a)&&Number.isFinite(b)&&a!==b)return true;
  }
  return false;
}
async function upsertSummaryRows(rows){
  if(!rows.length)return;
  await sb('seller_orders?on_conflict=user_id,order_number',{
...
async function hasSearchPageWork(userId,windowFrom,windowTo,pageFrom){
  const rows=await activeProbeJobs(userId);
  return (rows||[]).some(j=>kind(j)==='order_search' &&
    String(j.payload_json?.windowFrom||'')===String(windowFrom||'') &&
    String(j.payload_json?.windowTo||'')===String(windowTo||'') &&
    Number(j.payload_json?.pageFrom||0)===Number(pageFrom||0) &&
    (['queued','claimed','running'].includes(j.status)||(j.status==='completed'&&!j.progress_json?.orchestratedAt))
  );
}
async function queueNextSearchPage(job,nextFrom){
  const payload=job.payload_json||{};
  if(await hasSearchPageWork(job.user_id,payload.windowFrom,payload.windowTo,nextFrom))return false;
  const body={...(payload.probe?.body||{}),from:nextFrom};
  const now=new Date().toISOString();
  await sb('collector_jobs',{method:'POST',body:[{
    user_id:job.user_id,
    ...
    payload_json:{
      ...
      windowFrom:payload.windowFrom,windowTo:payload.windowTo,pageFrom:nextFrom,pageSize:Number(body.size||1000),
      probe:{mode:'fetch_json',method:'POST',url:'https://order-management-api.tcgplayer.com/orders/search?api-version=2.0',body}
    },
    progress_json:{stage:'queued',percent:0,detail:`Seller History order-summary page ${nextFrom} queued`,updatedAt:now},
    max_attempts:3
  }],prefer:'return=minimal'});
  return true;
}
async function queueDetailJobs(searchJob,orderNumbers){
  const unique=[...new Set((orderNumbers||[]).map(String).filter(Boolean))];
  if(!unique.length)return 0;
  const existing=await activeProbeJobs(searchJob.user_id);
  const active=new Set((existing||[]).filter(j=>kind(j)==='order_detail'&&['queued','claimed','running'].includes(j.status))
    .map(j=>String(j.payload_json?.orderNumber||'')));
  const toQueue=unique.filter(id=>!active.has(id));
  const now=new Date().toISOString();
  let queued=0;
  for(let i=0;i<toQueue.length;i+=100){
    const batch=toQueue.slice(i,i+100).map(orderNumber=>({
      user_id:searchJob.user_id,
      source:'agent',action:'seller_portal_readonly_probe',status:'queued',priority:6,
      required_capability:'tcgplayer_authenticated_session',preferred_executor:'android_agent',
      payload_json:{
        sellerHistoryKind:'order_detail',sellerHistoryParentJobId:searchJob.job_id,orderNumber,
        probe:{mode:'fetch_json',method:'GET',url:`https://order-management-api.tcgplayer.com/orders/${encodeURIComponent(orderNumber)}?api-version=2.0`}
      },
      progress_json:{stage:'queued',percent:0,detail:`Seller History detail probe queued for ${orderNumber}`,updatedAt:now},
      max_attempts:3
    }));
...
async function processSearchJob(job){
  ...
  if(!checked.ok){
    await markOrchestrated(job,{orchestratorStatus:'search_shape_rejected',orchestratorError:checked.error});
    return {accepted:false,detailsQueued:0,nextQueued:false};
  }
  const payload=job.payload_json||{};
  const collectedAt=new Date().toISOString();
  const existing=await fetchExistingWindow(job.user_id,payload.windowFrom,payload.windowTo);
  const existingById=new Map((existing||[]).map(r=>[String(r.order_number),r]));
  const normalized=checked.orders.map(o=>normalizeSummaryOrder(job.user_id,o,collectedAt)).filter(Boolean);
  const details=[];
  for(let i=0;i<checked.orders.length;i++){
    const raw=checked.orders[i],row=normalized.find(r=>r.order_number===detailOrderNumberFromSearchRow(raw));
    if(!row)continue;
    const current=existingById.get(row.order_number);
    if(summaryNeedsDetail(raw,row,current))details.push(row.order_number);
  }
  await upsertSummaryRows(normalized);
  const detailsQueued=await queueDetailJobs(job,details);
  const pageFrom=Number(payload.pageFrom??payload.probe?.body?.from??0);
  const returned=checked.orders.length;
  let nextQueued=false;
  if(returned>0 && pageFrom+returned<checked.totalOrders)nextQueued=await queueNextSearchPage(job,pageFrom+returned);
  const incomplete=returned===0 && pageFrom<checked.totalOrders;
  await markOrchestrated(job,{
    orchestratorStatus:incomplete?'search_pagination_stalled':'search_normalized',
    searchTotalOrders:checked.totalOrders,searchReturned:returned,summaryRowsUpserted:normalized.length,
    detailJobsQueued:detailsQueued,nextPageQueued:nextQueued
  });
...
async function upsertDetailBundle(job){
  ...
  await sb('seller_orders?on_conflict=user_id,order_number',{
    method:'POST',body:[bundle.order],prefer:'resolution=merge-duplicates,return=minimal'
  });
  if(bundle.items.length)await sb('seller_order_items?on_conflict=user_id,row_id',{
    method:'POST',body:bundle.items,prefer:'resolution=merge-duplicates,return=minimal'
  });
  if(bundle.refunds.length)await sb('seller_refunds?on_conflict=user_id,refund_id',{
    method:'POST',body:bundle.refunds,prefer:'resolution=merge-duplicates,return=minimal'
  });
  if(bundle.review)await sb('seller_reviews?on_conflict=user_id,order_number',{
    method:'POST',body:[bundle.review],prefer:'resolution=merge-duplicates,return=minimal'
  });
  await markOrchestrated(job,{
    orchestratorStatus:'order_detail_normalized',normalizedOrderNumber:bundle.order.order_number,
    normalizedItems:bundle.items.length,normalizedRefunds:bundle.refunds.length,normalizedReview:Boolean(bundle.review)
  });
...
async function main(){
  ...
  for(const job of completed||[]){
    ...
    try{
      ...
      if(k==='order_detail'){
        await upsertDetailBundle(job);
        detailProcessed++;
        continue;
      }
      other++;
    }catch(error){
      await markOrchestrated(job,{orchestratorStatus:'normalization_failed',orchestratorError:String(error?.message||error)});
    }
  }
