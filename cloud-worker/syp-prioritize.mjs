...
const jobs=await sb('collector_jobs?select=job_id,priority,payload_json&source=eq.agent&action=eq.seller_portal_readonly_probe&status=eq.queued&order=created_at.asc&limit=300');
let changed=0;
