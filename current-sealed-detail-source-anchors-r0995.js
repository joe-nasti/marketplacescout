// Scout Sealed source anchors: real <a> elements so Android WebView/browser handoff is reliable.
(() => {
  const cache=new Map();
  ...
  const priceShown=el=>el&&el.textContent.trim()&&el.textContent.trim()!=='—';

  function wrap(el,url,title,{always=false}={}){
    if(!el||!url||el.dataset.cxAnchorLinked==='1')return;
    if(!always&&!priceShown(el))return;
    const a=document.createElement('a');
    a.href=url;a.target='_blank';a.rel='noopener';a.className='cx-source-anchor';a.title=title||'Open source';
  ...
  async function decorate(){
    ...
    if(d.dataset.cxSourceAnchorsUuid===uuid)return;
    const {cards,price}=await load(uuid);if(selectedUuid()!==uuid)return;
    d.dataset.cxSourceAnchorsUuid=uuid;
