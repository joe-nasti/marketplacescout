...
for(const token of ['ask_collectish_supply_family_skus_v1','market-supply-sync','canonical_target_count','family_resolved_market_sync_failed','partial','supply ladder','printingMarketPrice','compactSafe','ladderSections','ask_collectish_family_printing_metadata_v1','rarityLabel','Rare','Mythic','opportunityRows','scarcity','premium','gap','WORTH_INVESTIGATING','SCARCE_ALREADY_PRICED','Buy-side watch','Research flag only','Open all printings in Collectish Scout','oracle=','marketDepth=1'])if(!presenter.includes(token))throw new Error(`missing deterministic Discord supply/opportunity token: ${token}`);
...
if(!/\*\*Nonfoil\*\*/.test(presenter)||!/\*\*Foil\*\*/.test(presenter))throw new Error('supply ladder must bucket nonfoil and foil variants');
