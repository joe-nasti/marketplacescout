...
for(const token of ['ask_collectish_supply_family_skus_v1','mtgjson_tcgplayer_skus','scryfall_oracle_id','LIGHTLY PLAYED','service_role'])if(!canonical.includes(token))throw new Error(`missing canonical MTGJSON family resolver token: ${token}`);
if(!/upper\(coalesce\(s\.condition,''\)\) in \('NEAR MINT','LIGHTLY PLAYED','NM','LP'\)/.test(canonical))throw new Error('canonical family resolver is not NM/LP scoped');
for(const token of ['ask_collectish_family_supply_concentration_v1','CARD_FAMILY_NM_LP_PRODUCT_FINISH_CONCENTRATION','top1_supply_share_pct','top3_supply_share_pct','hhi','tightest_printing'])if(!concentration.includes(token))throw new Error(`missing concentration token: ${token}`);
if(!/group by l\.product_id,l\.finish_scope/.test(concentration))throw new Error('concentration must separate product + finish');
for(const token of ['ask_collectish_family_supply_trend_v1','CARD_FAMILY_NM_LP_OBSERVED_HISTORY','SINGLE_COMPLETE_POINT','TIGHTENING','LOOSENING','daily_points'])if(!history.includes(token))throw new Error(`missing history token: ${token}`);
...
if(/ask_delvin_card_family_supply_targets_v1/.test(presenter))throw new Error('Discord supply presenter must not use Scout-catalog-only family targets');
...
if(!/handled:true/.test(presenter)||!/canonical_family_not_found/.test(presenter))throw new Error('supply questions must not fall through to generic printing clarification');
for(const token of ['scryfall_oracle_id','availability','ENGLISH','tcgplayer_product_id'])if(!family.includes(token))throw new Error(`missing Oracle product boundary token: ${token}`);
if(!/desired_conditions/.test(discovery))throw new Error('signed-in Scout discovery lost multi-condition support');
