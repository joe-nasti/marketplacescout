...
const path='supabase/functions/ask-collectish-delvin-supply-present/index.ts';
...
must(oldCompact,'compactRow');
s=s.replace(oldCompact,newCompact);
...
`;
must('\nDeno.serve(async req=>','Deno serve');
...
must(oldPromise,'analytics Promise');
s=s.replace(oldPromise,newPromise);

const oldRows="const rows=Array.isArray(concentration?.printing_rows)?concentration.printing_rows:[],opps=opportunityRows(rows,marketContext,metadata),oppText=opportunityText(opps,marketContext);";
const newRows="const rows=Array.isArray(concentration?.printing_rows)?concentration.printing_rows:[],opps=opportunityRows(rows,marketContext,metadata),oppText=opportunityTextV3(opportunity)||opportunityText(opps,marketContext);";
must(oldRows,'opportunity use');
...
must(oldFoot,'buy-side footer');
s=s.replace(oldFoot,newFoot);

fs.writeFileSync(path,s);
console.log('Patched Delvin opportunity v3 surface');
