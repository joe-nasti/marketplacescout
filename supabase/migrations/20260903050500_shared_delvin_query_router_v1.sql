...
begin
  ...
  select * into r
  from public.delvin_query_registry d
  where d.enabled
    and (
      lower(regexp_replace(d.prompt,'[^a-zA-Z0-9]+',' ','g')) = q
      or exists (
        select 1 from unnest(d.aliases) a
        where lower(regexp_replace(a,'[^a-zA-Z0-9]+',' ','g')) = q
      )
      or exists (
        select 1 from unnest(d.aliases) a
        where length(regexp_replace(a,'[^a-zA-Z0-9]+',' ','g')) >= 8
          and q like '%'||lower(regexp_replace(a,'[^a-zA-Z0-9]+',' ','g'))||'%'
      )
    )
  order by d.sort_order, d.query_key
  limit 1;

  if found then
    c := public.get_delvin_query_cache_v1(r.query_key);
    return jsonb_build_object(
      'handled',true,'route','delvin_query_cache','query_key',r.query_key,'prompt',r.prompt,
      'category',r.category,'payload',c,'followups',r.followups,
      'surface_type','delvin_query','source','shared_registry_cache'
    );
  end if;

  if q ~ '(across sets|cross set|which sets|best sets|treatment)' then
    treatment := case
      when q like '%textured foil%' then 'Textured Foil'
      when q like '%serialized%' then 'Serialized'
      when q like '%raised foil%' then 'Raised Foil'
      when q like '%fracture foil%' then 'Fracture Foil'
      when q like '%confetti foil%' then 'Confetti Foil'
      when q like '%galaxy foil%' then 'Galaxy Foil'
      when q like '%surge foil%' then 'Surge Foil'
      when q like '%rainbow foil%' then 'Rainbow Foil'
      when q like '%halo foil%' then 'Halo Foil'
      when q like '%gilded foil%' then 'Gilded Foil'
      when q like '%etched foil%' then 'Etched Foil'
      when q like '%borderless foil%' then 'Borderless Foil'
      when q like '%showcase foil%' then 'Showcase Foil'
      else null end;
  ...
  select upper(m[1]) into setcode
  from regexp_matches(upper(coalesce(p_question,'')), '\m([A-Z0-9]{3,6})\M', 'g') m
  where exists (select 1 from public.scout_opportunities_v5_cache s where upper(s.set_code)=upper(m[1]))
  limit 1;
  if setcode is not null and q ~ '(set|cards|treatment|foil|collector|market|doing|opportunit)' then
    if treatment is not null then
  ...
  if q like 'investigate %' or q like 'deep dive %' then
    ...
    if card<>'' then
      return jsonb_build_object('handled',true,'route','card_investigation','surface_type','card_investigation','card_name',card,
        'payload',public.ask_delvin_card_investigation_v1(card),'source','shared_deterministic_rpc');
    end if;
  end if;
  if q like 'all printings of %' or q like 'printing family %' then
    ...
    if card<>'' then
      return jsonb_build_object('handled',true,'route','printing_family','surface_type','printing_family','card_name',card,
        'payload',public.ask_delvin_printing_family_v1(card,least(greatest(coalesce(p_limit,30),1),100)),'source','shared_deterministic_rpc');
    end if;
  end if;
