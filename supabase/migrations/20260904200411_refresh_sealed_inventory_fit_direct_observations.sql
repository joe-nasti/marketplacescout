...
begin
  with latest as (
    ...
    join public.marketplace_scans s
      on s.user_id = r.user_id and s.scan_id = r.scan_id
    where s.set_slug = 'sealed-precon-direct-refresh'
      and r.raw_json->>'coverage' = 'COMPLETE'
      and r.direct_available is not null
    order by r.user_id, r.sku_id, s.captured_at desc
  )
...
comment on function public.refresh_sealed_inventory_fit_direct_observations() is
  'Hydrates resolved sealed component identities and complete exact-SKU Direct observations into the inventory-fit current cache.';

select public.refresh_sealed_inventory_fit_direct_observations();
