...
begin
  ...
  insert into public.source_captures(
    ...
    new.user_id, 'MTGStocks', 'discovery_candidate', 'canonical:' || new.intel_id::text,
...
drop trigger if exists trg_mirror_canonical_mtgstocks_interest_capture on public.market_intel_items;
create trigger trg_mirror_canonical_mtgstocks_interest_capture
after insert or update of metadata_json, observed_at, title, summary
on public.market_intel_items
for each row
when (new.source_name = 'MTGStocks' and new.source_subtype = 'interests')
execute function public.mirror_canonical_mtgstocks_interest_capture();

with canonical as (
  select *
  from public.market_intel_items
  where source_name='MTGStocks'
    and source_subtype='interests'
    and coalesce(metadata_json->>'source_date','') <> ''
), purged as (
  delete from public.source_captures s
  using (select distinct user_id, metadata_json->>'source_date' as source_date from canonical) c
  where s.user_id=c.user_id
    and s.source='MTGStocks'
    and s.capture_type='discovery_candidate'
    and s.payload_json->>'date'=c.source_date
    and s.payload_json->>'discovery_source'='telegram_announcement'
  returning s.capture_id
)
insert into public.source_captures(
