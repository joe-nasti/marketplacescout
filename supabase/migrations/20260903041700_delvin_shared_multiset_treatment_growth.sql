...
begin
  ...
  treatment := case
    ...
    when q like '%oil slick%' then 'Oil Slick Raised Foil'
    when q like '%step and compleat%' then 'Step-and-Compleat Foil'
    when q like '%neon ink%' then 'Neon Ink'
    when q like '%gilded foil%' then 'Gilded Foil'
    when q like '%etched foil%' then 'Etched Foil'
    when q like '%retro frame foil%' then 'Retro Frame Foil'
    when q like '%borderless foil%' then 'Borderless Foil'
    when q like '%showcase foil%' then 'Showcase Foil'
    when q like '%extended art foil%' then 'Extended Art Foil'
    else null end;
  ...
  setcode:=setcodes[1];
  if treatment is not null and coalesce(array_length(setcodes,1),0)>=2 and q ~ '(growth|upside|room to grow|growth left|still grow|more room|more upside|plateau|ceiling|run left|legs left)' then
    return jsonb_build_object('handled',true,'route','treatment_growth_comparison','surface_type','treatment_growth_comparison','set_codes',setcodes,'treatment',treatment,
      'payload',public.ask_delvin_treatment_growth_v1(setcodes,treatment,7),'source','shared_deterministic_rpc');
  end if;
  ...
  if setcode is not null and q ~ '(set|cards|treatment|foil|collector|market|doing|opportunit|growth|upside|moving|movers)' then
