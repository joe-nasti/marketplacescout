...
returns table(
  ...
  coverage_state text, acquisition_price numeric, acquisition_price_source text,
  acquisition_observed_at timestamptz, acquisition_age_hours numeric,
  acquisition_observation_status text, direct_first_net_ev numeric,
...
language sql stable security invoker set search_path=public as $$
  with shipping_price as (
    select distinct on (p.sealed_uuid)
      ...
      p.source::text acquisition_price_source,
      p.captured_at acquisition_observed_at
  ...
  select c.sealed_uuid,c.product_name,c.set_code,c.category,c.coverage_state,
    p.acquisition_price,p.acquisition_price_source,p.acquisition_observed_at,
    round((extract(epoch from (now()-p.acquisition_observed_at))/3600)::numeric,1),
    case
      when p.acquisition_observed_at>=now()-interval '24 hours' then 'CURRENT'
      when p.acquisition_observed_at>=now()-interval '72 hours' then 'AGING'
      else 'STALE'
    end,
...
comment on function public.ask_delvin_sealed_direct_crack_v1(integer) is
  'Shipping-aware sealed recommendations using TCG Low + shipping only, with acquisition observation source and freshness.';
