...
with velocity_raw as (
  ...
  select lower(c.card_name) card_key,lower(c.set_name) set_key,c.price_bucket,
    min(coalesce(p.lowest_listing_price,
      case when p.low_price is not null then p.low_price+coalesce(p.lowest_shipping,0) end)) current_low_with_shipping,
    min(p.observed_at) oldest_current_observed_at,
    max(p.observed_at) newest_current_observed_at,
    count(*) filter(where coalesce(p.lowest_listing_price,p.low_price+coalesce(p.lowest_shipping,0)) is not null) official_priced_skus
  from catalog c
  join public.tcgplayer_official_sku_price_current p on p.sku_id=c.sku_id
  group by 1,2,3
), eval_rows as (
  ...
  select lower(card_name) card_key,lower(set_name) set_key,price_bucket,
    min(current_low_with_shipping) scout_current_low_with_shipping,
    ...
    count(*) filter(where current_low_with_shipping is not null) scout_priced_skus,
  ...
  select m.card_name,m.base_card_name,m.set_name,m.set_code,m.price_bucket,m.sales_month,m.current_rank,m.previous_rank,m.rank_improvement,
    ...
    coalesce(oc.current_low_with_shipping,fe.scout_current_low_with_shipping) current_low_with_shipping,
    case when oc.current_low_with_shipping is not null then 'TCGPLAYER_OFFICIAL_LOW_WITH_SHIPPING' when fe.scout_current_low_with_shipping is not null then 'SCOUT_LOW_WITH_SHIPPING' else null end current_price_source,
    oc.official_priced_skus,fe.scout_priced_skus,
    fe.first_low_with_shipping,round(fe.median_price_change_pct::numeric,2) recent_price_change_pct,
    fe.current_direct_available,fe.first_direct_available,fe.current_direct_listings,fe.first_direct_listings,
    fe.price_history_start,fe.price_history_end,
    round((extract(epoch from(fe.price_history_end-fe.price_history_start))/3600.0)::numeric,2) price_history_hours,
    case when m.average_sale_price>0 and coalesce(oc.current_low_with_shipping,fe.scout_current_low_with_shipping) is not null
      then round(((coalesce(oc.current_low_with_shipping,fe.scout_current_low_with_shipping)/m.average_sale_price)-1)*100,2) end current_low_vs_report_avg_pct,
  ...
  left join official_current oc on oc.card_key=lower(m.card_name) and oc.set_key=lower(m.set_name) and oc.price_bucket=m.price_bucket
  ...
  select count(*) high_demand_rows,
    ...
    count(*) filter(where identity_scope='EXACT_PRODUCT' and current_low_with_shipping is not null) exact_product_price_rows,
    count(*) filter(where identity_scope='SET_CARD_FAMILY' and current_low_with_shipping is not null) family_price_rows
...
select jsonb_build_object(
  ...
  'scope','TCGplayer Top Selling demand joined to Collectish NM/LP TCG Low + shipping and supply evidence',
  'price_note','Current price uses TCGplayer official lowest listing / Low + shipping where available; no Market-price fallback is used.',
