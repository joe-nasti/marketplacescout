...
base as (
  select
    ...
    coalesce(c.tcg_market, 0) * c.quantity as market_value
  from public.sealed_component_ev_current c
...
summary as (
  select
    ...
    count(*) filter (where syp_eligible and coalesce(direct_available_current, 0) = 0)::int as syp_no_direct,
    coalesce(sum(practical_value) filter (where syp_eligible and coalesce(direct_available_current, 0) = 0), 0)::numeric as syp_no_direct_value,
...
select jsonb_build_object(
  ...
  'notes', jsonb_build_array(
    ...
    'Unresolved SKU identities are reported separately and are not counted as zero inventory or zero growth.'
  )
)
from summary s
cross join labels
