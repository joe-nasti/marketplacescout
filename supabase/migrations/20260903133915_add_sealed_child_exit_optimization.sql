-- Pick one executable exit per sealed child: crack it or sell it sealed.
-- The parent adjustment is relative to the child value already present in the
-- executable cache, so modeled crack EV is replaced rather than added twice.
...
select d.*,
  ...
  'one child unit chooses max(current crack net, current sealed-sale net); fixed children remain comparison-only'::text decision_policy
from decided d;
...
select c.user_id,c.sealed_uuid,
  ...
  'base practical EV plus only the child-route improvement; never crack plus sealed resale'::text optimization_policy
...
grant select on public.sealed_product_exit_optimized_current to authenticated,service_role;
revoke all on public.sealed_child_exit_decision_current from anon;
revoke all on public.sealed_product_exit_optimized_current from anon;
