...
returns table(
  ...
  creator_lane text,
  enabled boolean,
  videos_discovered bigint,
  videos_completed bigint,
  videos_pending bigint,
  videos_failed bigint,
  no_native_captions bigint,
  completion_pct numeric,
  signal_yield_pct numeric,
  latest_discovered_at timestamptz,
...
as $$
with subs as (
  select sc.user_id, sc.source as channel_name,
         sc.payload_json->>'channel_id' as channel_id,
         coalesce(sc.payload_json->>'creator_lane','general') as creator_lane,
         coalesce((sc.payload_json->>'enabled')::boolean,true) as enabled
  from source_captures sc where sc.capture_type='video_subscription'
), agg as (
  select l.user_id,l.channel_id,count(*) videos_discovered,
         count(*) filter(where l.status in ('evaluated','no_signal')) videos_completed,
         count(*) filter(where l.status in ('discovered','queued','transcript_ready','unavailable_native')) videos_pending,
         count(*) filter(where l.status='failed') videos_failed,
         count(*) filter(where l.status='unavailable_native') no_native_captions,
         max(l.discovered_at) latest_discovered_at,max(l.evaluated_at) latest_evaluated_at,
         max(coalesce(l.evaluated_at,l.last_attempt_at)) filter(where l.status in ('evaluated','no_signal')) latest_success_at,
  ...
  select l.user_id,l.channel_id,l.status,
         row_number() over(partition by l.user_id,l.channel_id order by coalesce(l.last_attempt_at,l.updated_at,l.discovered_at) desc) rn
  from market_intel_youtube_video_ledger l
), failstreak as (
  select user_id,channel_id,
         case when max(case when rn=1 then status end)='failed'
              then count(*) filter(where rn<=3 and status='failed')::int else 0 end consecutive_failures
  from ranked group by user_id,channel_id
), outcomes as (
  select user_id,lower(source_name) source_name,measured_signals,predictive_pct,reactive_pct,confirming_pct
  from market_intel_source_outcomes
)
...
select r.user_id,r.channel_id,r.target_count,r.status,coalesce(h.scheduler_priority,0),r.requested_at
from market_intel_youtube_backfill_requests r
left join admin_youtube_channel_health() h on h.user_id=r.user_id and h.channel_id=r.channel_id
where r.status='active'
order by coalesce(h.scheduler_priority,0) desc,r.requested_at asc;
$$;
