...
drop view if exists public.sealed_product_trajectory_features_current;

create or replace view public.sealed_product_trajectory_features_current
with (security_invoker=true) as
with daily as (
  select b.user_id,p.uuid sealed_uuid,p.category,p.subtype,case when p.name ilike '%case%' then 'case' else 'box' end package_form,coalesce(p.release_date,sc.released_at) release_date,b.bucket_start_date,
  ...
  group by b.user_id,p.uuid,p.category,p.subtype,case when p.name ilike '%case%' then 'case' else 'box' end,coalesce(p.release_date,sc.released_at),b.bucket_start_date
), rollup as (
  select d.user_id,d.sealed_uuid,d.category,d.subtype,d.package_form,d.release_date,
  ...
  from daily d group by d.user_id,d.sealed_uuid,d.category,d.subtype,d.package_form,d.release_date
)
...
with pairs as (
  ...
  join public.sealed_product_trajectory_features_current a
    ...
    and a.category=t.category and a.subtype=t.subtype and a.package_form=t.package_form
