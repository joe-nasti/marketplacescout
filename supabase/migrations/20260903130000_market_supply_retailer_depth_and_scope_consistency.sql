-- Extend exact-SKU market supply with true retailer stock/depth while preserving
-- the rule that global_supply_classification is earned only by a fresh, complete
-- all-TCGplayer exact-SKU snapshot.
    ...
    select coalesce(jsonb_agg(
      case
        ...
        then v || jsonb_build_object(
          'global_supply_classification',coalesce(s->>'global_supply_classification','UNPROVEN'),
          'supply_scope',case when coalesce((s->>'available')::boolean,false) then 'EXACT_SKU_ALL_TCGPLAYER' else coalesce(v->>'supply_scope','DIRECT_ONLY') end,
          'supply_scope_note',case when coalesce((s->>'available')::boolean,false)
            then 'Market-wide supply uses exact-SKU TCGplayer marketplace listing depth; Direct remains a subset. External retailer prices indicate availability only unless stock counts are collected.'
            else coalesce(v->>'supply_scope_note','Direct inventory alone cannot prove market-wide supply.') end,
          'market_supply',s
        )
        else v
      end
