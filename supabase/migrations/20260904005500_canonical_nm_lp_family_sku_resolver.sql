...
with seed as (
  ...
  select s.product_id::text,s.sku_id::text,c.name as card_name,c.set_code,c.collector_number,
         upper(coalesce(nullif(s.printing,''),nullif(s.finish,''),'UNKNOWN')) as printing,
         upper(coalesce(s.condition,'')) as condition,
         upper(coalesce(s.language,'ENGLISH')) as language,
         c.scryfall_id,c.scryfall_oracle_id
  from cards c
...
select distinct on (sku_id)
  product_id,sku_id,card_name,set_code,collector_number,printing,
...
from rows
order by sku_id,product_id,printing;
$$;
...
comment on function public.ask_collectish_supply_family_skus_v1(text,text) is 'Canonical English paper NM/LP Oracle-family TCGplayer SKU resolver backed by MTGJSON. Used by deterministic supply analysis without invoking on-demand SKU discovery.';
