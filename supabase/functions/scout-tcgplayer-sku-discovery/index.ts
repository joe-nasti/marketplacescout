...
async function rest(path: string, init: RequestInit = {}) {
  const r = await fetch(`${urlBase()}/rest/v1/${path}`, { ...init, headers: { apikey: serviceKey(), Authorization: `Bearer ${serviceKey()}`, "Content-Type": "application/json", ...(init.headers || {}) } });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const body = await req.json().catch(() => ({})); const productId = String(body?.product_id || body?.productId || "").trim(); const desiredFinish = body?.desired_finish ? String(body.desired_finish) : body?.printing ? String(body.printing) : null; const desiredCondition = normCondition(body?.desired_condition || body?.condition || "NEAR MINT"); const desiredLanguage = normLanguage(body?.desired_language || body?.language || "ENGLISH"); const force = Boolean(body?.force); const persist = body?.persist !== false; const reason = String(body?.reason || "on_demand_sku_discovery"); if (!/^\d+$/.test(productId)) return json({ error: "product_id is required" }, 400);
    const anchor = await anchorForProduct(productId); if (!anchor?.mtgjson_uuid) return json({ error: "Known MTG card identity is required before SKU discovery", product_id: productId }, 404);
    const freshCutoff = new Date(Date.now() - 6*60*60*1000).toISOString(); let cached = await rest(`scout_tcgplayer_sku_discovery_cache?product_id=eq.${encodeURIComponent(productId)}&last_seen_at=gte.${encodeURIComponent(freshCutoff)}&select=*&order=sku_id.asc`).catch(() => []); if (!Array.isArray(cached)) cached = [];
    ...
    if (force || !cached.length || (desiredFinish && !cachedMatch)) {
      const fetched = await fetchSkus(productId); endpoint = fetched.endpoint; source = fetched.source; fallbackReason = fetched.fallback_reason; const now = new Date().toISOString();
      ...
      if (persist && records.length) await rest("scout_tcgplayer_sku_discovery_cache?on_conflict=sku_id", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: records });
    }
    const english = records.filter((x: any) => !x.language || x.language === "ENGLISH"), materialized: any[] = [];
    if (persist) for (const x of english) { if (x.condition !== "NEAR MINT") continue; const row = { sku_id: x.sku_id, mtgjson_uuid: anchor.mtgjson_uuid, product_id: productId, scryfall_id: anchor.scryfall_id, card_name: anchor.card_name, set_code: anchor.set_code, collector_number: anchor.collector_number, printing: x.printing, finish: x.variant || null, condition: "NEAR MINT", language: "ENGLISH", release_date: anchor.release_date, source_updated_at: new Date().toISOString() }; await rest("scout_card_catalog?on_conflict=sku_id", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: row }); materialized.push(row); await rest(`scout_tcgplayer_sku_discovery_cache?sku_id=eq.${encodeURIComponent(x.sku_id)}`, { method: "PATCH", body: { catalog_materialized_at: new Date().toISOString() } }).catch(() => null); }
    const wanted = english.filter((x: any) => finishMatches(x.printing, desiredFinish) && (!desiredCondition || x.condition === desiredCondition) && (!desiredLanguage || x.language === desiredLanguage)); const outcome = discoveryOutcome(records, wanted, desiredFinish, desiredCondition, desiredLanguage); const queued: any[] = [], authHeader = req.headers.get("Authorization"); for (const x of wanted) { if (x.condition !== "NEAR MINT") continue; const q = await queueRefreshAsCaller(authHeader, x.sku_id, reason); if (q) queued.push({ sku_id: x.sku_id, ...q }); }
    return json({ ok: true, product_id: productId, card: anchor, source_endpoint: endpoint, source, fallback_reason: fallbackReason, cache_used: !endpoint, discovered_count: records.length, english_count: english.length, materialized_nm_english_count: materialized.length, requested_finish: desiredFinish, requested_condition: desiredCondition, requested_language: desiredLanguage, outcome, matches: wanted, queued_refreshes: queued, available_nm_english_printings: [...new Set(english.filter((x: any) => x.condition === "NEAR MINT").map((x: any) => x.printing).filter(Boolean))] });
