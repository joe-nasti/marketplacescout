...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const cacheKey = negativeKey(productId, desiredFinish, desiredCondition, desiredLanguage), negative = !force && desiredFinish ? cachedRows.map((x: any) => validNegative(x, cacheKey)).find(Boolean) || null : null;
    ...
    if (!negative && (force || !cached.length || (desiredFinish && !cachedMatch))) {
    ...
    const wanted = english.filter((x: any) => finishMatches(x.printing, desiredFinish) && (!desiredCondition || x.condition === desiredCondition) && (!desiredLanguage || x.language === desiredLanguage)); const outcome = negative?.outcome || discoveryOutcome(records, wanted, desiredFinish, desiredCondition, desiredLanguage);
    ...
    const queued: any[] = [], authHeader = req.headers.get("Authorization"); for (const x of wanted) { if (x.condition !== "NEAR MINT") continue; const q = await queueRefreshAsCaller(authHeader, x.sku_id, reason); if (q) queued.push({ sku_id: x.sku_id, ...q }); }
