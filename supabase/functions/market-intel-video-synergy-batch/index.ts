...
async function rest(p:string,o:any={}){
 const r=await fetch(`${U}/rest/v1/${p}`,{method:o.method||'GET',headers:{...H(),...(o.prefer?{Prefer:o.prefer}:{})},body:o.body===undefined?undefined:JSON.stringify(o.body)});
...
async function serviceAuth(t:string){
 if(!t)return false;if(S&&t===S)return true;
 try{const r=await fetch(`${U}/auth/v1/admin/users?page=1&per_page=1`,{headers:{apikey:t,Authorization:`Bearer ${t}`}});return r.ok}catch{return false}
}
...
async function oa(prompt:string){
 const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${O}`,'Content-Type':'application/json'},body:JSON.stringify({model:M,store:false,input:prompt,max_output_tokens:2200})});
...
async function card(name:string){
 for(const mode of ['exact','fuzzy']){try{const r=await fetch(`https://api.scryfall.com/cards/named?${mode}=${encodeURIComponent(name)}`,{headers:{'User-Agent':'MarketplaceScout/1.3'}});if(r.ok){const c=await r.json();if(c?.id)return{name:String(c.name),id:String(c.id),set:String(c.set||''),released_at:String(c.released_at||'')}}}catch{}}
 return null;
}
function hit(text:string){for(const [re,p,label] of T as any[])if(re.test(text))return{p,label};return null}
async function resolveBatch(batch:any[],title:string){
 if(!batch.length)return [];
 const passages=batch.map(h=>`ID ${h.id} [${h.label}]\nCONTEXT: ${h.context}\nCURRENT: ${h.evidence}`).join('\n\n---\n\n');
 const d=await oa(`Extract explicit Magic: The Gathering new-card -> existing-card deck-building relationships from these Commander review passages. Return ONLY compact JSON {"relationships":[{"id":integer,"source_card":string,"target_card":string,"relationship_type":"new_card_synergy"|"upgrade_for"|"combo_with"|"enabler_for"|"payoff_for"|"other","confidence":number,"summary":string}]}. source_card is the NEW/UPCOMING commander/card whose deck is being discussed. target_card is the DIFFERENT, already-existing printed card explicitly recommended because of it. Skip passages that only praise the new card itself. Do not invent cards. One relationship max per passage. Keep each summary under 18 words. Video=${title}\n\n${passages}`);
 return Array.isArray(d?.relationships)?d.relationships:[];
}
...
Deno.serve(async req=>{
 ...
 try{
  ...
  for(let i=0;i<s.length;i++){const h=hit(String(s[i]?.text||''));if(h)hits.push({i,start_ms:Math.max(0,Math.round(Number(s[i]?.offset)||0)),prominence:h.p,label:h.label,evidence:trim(s[i]?.text,1300),context:trim([s[i-3]?.text,s[i-2]?.text,s[i-1]?.text].filter(Boolean).join(' '),2200)})}
  hits.sort((a,b)=>b.prominence-a.prominence||a.start_ms-b.start_ms);
  const selected:any[]=[];for(const h of hits){if(selected.some(x=>Math.abs(x.start_ms-h.start_ms)<18000))continue;selected.push({...h,id:selected.length});if(selected.length>=16)break}
  ...
  for(const rel of resolved){
   const h=selected.find(x=>x.id===Number(rel.id));
   if(!h||!rel.source_card||!rel.target_card||norm(rel.source_card)===norm(rel.target_card)){rejected++;continue}
   const [src,tgt]=await Promise.all([card(rel.source_card),card(rel.target_card)]);
   if(!src||!tgt||src.id===tgt.id||(tgt.released_at&&tgt.released_at>date)){rejected++;continue}
   const cat=await rest(`scout_card_catalog?select=sku_id&scryfall_id=eq.${encodeURIComponent(tgt.id)}&limit=1`).catch(()=>[]);
   const isActionable=Array.isArray(cat)&&cat.length>0,sourceFuture=!!src.released_at&&src.released_at>date,url=p.url||`https://www.youtube.com/watch?v=${p.video_id}`;
   const type=['new_card_synergy','upgrade_for','combo_with','enabler_for','payoff_for','other'].includes(String(rel.relationship_type))?String(rel.relationship_type):'new_card_synergy';
   const key=`synergy|${p.video_id}|${h.start_ms}|${norm(tgt.name)}|${norm(src.name)}`;
   let intelId:any=null;const ex=await rest(`market_intel_items?select=intel_id,metadata_json&user_id=eq.${owner}&source_url=eq.${encodeURIComponent(url)}&limit=200`).catch(()=>[]);intelId=(ex||[]).find((q:any)=>q?.metadata_json?.video_event_key===key)?.intel_id||null;
   if(!intelId){const ins=await rest('market_intel_items',{method:'POST',prefer:'return=representation',body:{user_id:owner,source_type:'youtube',source_name:cap.source||'YouTube',source_url:url,title:p.title||tgt.name,author:cap.source||null,summary:trim(rel.summary,500)||`${tgt.name} recommended for ${src.name}`,claim_type:'product',direction:'bullish',signal_stage:'leading',confidence:Math.max(.65,Math.min(.98,Number(rel.confidence)||.82)),published_at:p.published_at||null,source_profile:'creator_commander',source_subtype:'new_commander_synergy',metadata_json:{platform:'youtube',video_id:p.video_id,video_event_key:key,event_type:'new_commander_synergy',start_ms:h.start_ms,prominence:h.prominence,relationship_source_card:src.name,relationship_target_card:tgt.name,relationship_type:type,source_is_unreleased:sourceFuture,target_is_actionable:isActionable,resolution_method:'two_batch_relationship_context+scryfall'}}});intelId=ins?.[0]?.intel_id||null;if(intelId)await rest('market_intel_entities',{method:'POST',prefer:'return=minimal',body:{intel_id:intelId,user_id:owner,entity_type:'card',entity_name:tgt.name,scryfall_id:tgt.id,set_code:tgt.set,confidence:.94}}).catch(()=>null)}
  ...
  await rest(`source_captures?capture_id=eq.${cap.capture_id}&user_id=eq.${owner}`,{method:'PATCH',body:{metadata_json:{...(cap.metadata_json||{}),synergy_relationships_saved:saved,synergy_relationships_actionable:actionable,synergy_relationships_processed_at:new Date().toISOString(),synergy_relationship_trigger_hits:hits.length,synergy_relationship_candidates:selected.length,synergy_relationship_batch_version:4}}}).catch(()=>null);
