...
async function resolveOne(h:any,title:string){
 const prompt=`You extract Magic: The Gathering Commander-review card relationships from ONE praise passage. Identify only existing cards directly praised/recommended because of the new commander/card currently being reviewed. Return compact JSON {"relationships":[{"source_card":string,"target_card":string,"relationship_type":"new_card_synergy"|"upgrade_for"|"combo_with"|"enabler_for"|"payoff_for"|"other","confidence":number,"summary":string}]}. You may return multiple targets only when the praise passage clearly recommends a list/group. TARGET RULE: the target must be the named card the praise refers to; if wording says 'this card' use the closest preceding named card in context. SOURCE RULE: source_card is the new commander/card whose section is being discussed. Skip generic praise of the new commander itself. Skip negative/anti-synergy statements such as 'does not work'. Do not invent names. Summary <=14 words. Video=${title}\nLABEL=${h.label}\nSECTION CONTEXT: ${h.context}\nPRAISE PASSAGE: ${h.evidence}`;
 const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${O}`,'Content-Type':'application/json'},body:JSON.stringify({model:M,store:false,input:prompt,reasoning:{effort:'minimal'},max_output_tokens:1200})});
 ...
 const parsed=parseJson(out);const rels=Array.isArray(parsed?.relationships)?parsed.relationships:[];
 return rels.slice(0,6).map((x:any)=>({...x,id:h.id}));
}
async function resolveAll(selected:any[],title:string,concurrency=6){
 const out:any[]=[];let cursor=0;
 async function worker(){while(true){const i=cursor++;if(i>=selected.length)return;try{out.push(...await resolveOne(selected[i],title))}catch{}}}
...
Deno.serve(async req=>{
 ...
 try{
  ...
  for(let i=0;i<s.length;i++){
   ...
   hits.push({i,start_ms:Math.max(0,Math.round(Number(s[i]?.offset)||0)),prominence:h.p,label:h.label,evidence:trim(s[i]?.text,1700),context:trim(context.filter(Boolean).join(' '),7600)});
  }
  hits.sort((a,b)=>a.start_ms-b.start_ms||b.prominence-a.prominence);
  const selected:any[]=[];
  for(const h of hits){
    if(selected.some(x=>Math.abs(x.start_ms-h.start_ms)<10000)){const near=selected.find(x=>Math.abs(x.start_ms-h.start_ms)<10000);if(near&&h.prominence>near.prominence)Object.assign(near,h);continue}
  ...
  await rest(`source_captures?capture_id=eq.${cap.capture_id}&user_id=eq.${owner}`,{method:'PATCH',body:{metadata_json:{...(cap.metadata_json||{}),synergy_relationships_saved:saved,synergy_relationships_actionable:actionable,synergy_relationships_processed_at:new Date().toISOString(),synergy_relationship_trigger_hits:hits.length,synergy_relationship_candidates:selected.length,synergy_relationship_batch_version:11}}}).catch(()=>null);
