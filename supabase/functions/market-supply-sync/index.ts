...
Deno.serve(async req=>{
  ...
  for(let page=0;page<maxPages;page++){
    ...
    sourceTotal=Number(outer?.totalResults||sourceTotal||0);pages++;
    listings.push(...rows);
    from+=rows.length;
  ...
  const row={
    ...
    metadata:{
      ...
      methodology:'TCGplayer.com listings search; live channel-0 listings; exact productConditionId/SKU; banned/suspended channel exclusions applied.'
    }
  };
  const inserted=await rest('market_supply_snapshots',{method:'POST',prefer:'return=representation',body:[row]});
  return js({ok:true,snapshot:inserted?.[0]||row,identity:{product_id:productId,sku_id:skuId,...identity}});
});
