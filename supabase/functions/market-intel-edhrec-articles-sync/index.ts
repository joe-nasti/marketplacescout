...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    for(const sub of subs){
      ...
      const existingRows=await rest(t,`source_captures?select=source_key,metadata_json&user_id=eq.${encodeURIComponent(owner)}&source=eq.${encodeURIComponent(SOURCE)}&capture_type=eq.content_item&order=captured_at.desc&limit=1000`).catch(()=>[]);
      ...
      for(let page=1;page<=maxPages&&!stop;page++){
        ...
        for(const url of urls){
          ...
          try{const r=await ingest(t,item,owner,scheduler),s=Number(r?.saved||0),d=Number(r?.duplicates||0);saved+=s;duplicates+=d;out.saved+=s;out.duplicates+=d;if(captureId)await rest(t,`source_captures?capture_id=eq.${encodeURIComponent(captureId)}&user_id=eq.${encodeURIComponent(owner)}`,{method:'PATCH',prefer:'return=minimal',body:{metadata_json:{status:'saved',index_url:INDEX,source_profile:'commander_editorial',source_subtype:st,ingested_at:new Date().toISOString(),saved:s,duplicates:d}}});existing.set(url,{metadata_json:{status:'saved'}})}catch(e){failed++;out.failed++;out.errors.push(`${item.title}: ${(e as Error).message}`);if(captureId)await rest(t,`source_captures?capture_id=eq.${encodeURIComponent(captureId)}&user_id=eq.${encodeURIComponent(owner)}`,{method:'PATCH',prefer:'return=minimal',body:{metadata_json:{status:'failed',index_url:INDEX,source_profile:'commander_editorial',source_subtype:st,last_error:(e as Error).message,last_attempt_at:new Date().toISOString()}}}).catch(()=>null)}
        }
      }
