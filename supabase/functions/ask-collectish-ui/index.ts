...
function exactAction(upstream:any,type:string,screen?:string){return (upstream?.ui_actions||[]).find((a:any)=>a?.type===type&&(!screen||a?.screen===screen))||null}
...
function normalizeScout(row:any){return {
  sku_id:row?.sku_id??null,product_id:row?.product_id??null,product_name:row?.product_name??null,
  grade:row?.promoted_grade??null,score:row?.promoted_score??null,market_price:row?.sku_market_price??null,
  direct_available:row?.direct_available??null,edhrec_rank:row?.edhrec_rank??null,set_name:row?.set_name??null,
  condition:row?.condition??null,printing:row?.printing??null,language:row?.language??null
}}
...
async function buildSurfaces(t:string,body:any,upstream:any){
  ...
  try{
    if(screen==='scout'&&(ctx.product_id||ctx.sku_id)){
      const row=await rpc(t,'ask_collectish_get_scout_card',{p_product_id:ctx.product_id??null,p_sku_id:ctx.sku_id??null});
      const card=Array.isArray(row)?row[0]:row;
