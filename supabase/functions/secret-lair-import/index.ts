...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const regionRows=[];for(const rr of (b?.regions||[])){regionRows.push({release_id:releaseId,user_id:user.id,region:region(rr.region),storefront_url:clean(rr.storefront_url,2000)||null,currency:clean(rr.currency,8)||'USD',sale_start_at:rr.sale_start_at||release.sale_start_at||null,sale_end_at:rr.sale_end_at||release.sale_end_at||null,queue_start_at:rr.queue_start_at||null,order_limit_notes:clean(rr.order_limit_notes,2000)||null,shipping_notes:clean(rr.shipping_notes,2000)||null,allocation_notes:clean(rr.allocation_notes||rr.regional_supply_notes,2000)||null,local_demand_notes:clean(rr.local_demand_notes,2000)||null})}
