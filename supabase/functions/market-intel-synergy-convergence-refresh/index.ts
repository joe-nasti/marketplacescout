...
const inList=(vals:any[])=>`in.(${vals.map(v=>`\"${String(v).replaceAll('\\"','')}\"`).join(',')})`;
...
Deno.serve(async req=>{
  ...
  try{
    const rels=await rest(`market_intel_scout_synergy_opportunities?select=user_id,relationship_id,source_intel_id,source_name,target_card_name,oracle_id&synergy_printing_rank=eq.1&order=synergy_priority_score.desc&limit=${limit}`);
    let refreshed=0,totalSignals=0,multiSource=0,strong=0;const details:any[]=[];
    for(const r of rels||[]){
      if(!r.oracle_id)continue;
      ...
      const ci=catalyst?.[0]||{};const catalystAt=new Date(ci.published_at||ci.observed_at||ci.created_at||Date.now()).getTime();
      const windowStart=new Date(catalystAt-24*3600000).getTime(),windowEnd=new Date(catalystAt+7*86400000).getTime();
      const family=await rest(`mtgjson_cards?select=scryfall_id,tcgplayer_product_id&scryfall_oracle_id=eq.${enc(r.oracle_id)}&limit=500`).catch(()=>[]);
      const sf=[...new Set((family||[]).map((x:any)=>x.scryfall_id).filter(Boolean))],pids=[...new Set((family||[]).map((x:any)=>x.tcgplayer_product_id).filter(Boolean))];
      const entityIds=new Set<string>();
      for(let i=0;i<sf.length;i+=25){const chunk=sf.slice(i,i+25);const rows=await rest(`market_intel_entities?select=intel_id&scryfall_id=${enc(inList(chunk))}&user_id=eq.${enc(r.user_id)}&limit=1000`).catch(()=>[]);for(const e of rows||[])if(e.intel_id!==r.source_intel_id)entityIds.add(e.intel_id)}
      for(let i=0;i<pids.length;i+=25){const chunk=pids.slice(i,i+25);const rows=await rest(`market_intel_entities?select=intel_id&product_id=${enc(inList(chunk))}&user_id=eq.${enc(r.user_id)}&limit=1000`).catch(()=>[]);for(const e of rows||[])if(e.intel_id!==r.source_intel_id)entityIds.add(e.intel_id)}
      const items:any[]=[];const ids=[...entityIds];
      for(let i=0;i<ids.length;i+=40){const chunk=ids.slice(i,i+40);const rows=await rest(`market_intel_items?select=intel_id,source_name,source_type,source_profile,source_subtype,published_at,observed_at,created_at&intel_id=${enc(inList(chunk))}&user_id=eq.${enc(r.user_id)}&limit=1000`).catch(()=>[]);items.push(...(rows||[]))}
      ...
      const sources=[...new Set(valid.map(x=>x.source_name).filter(Boolean))];
      await rest('market_intel_synergy_convergence_cache',{method:'POST',prefer:'resolution=merge-duplicates,return=minimal',body:{user_id:r.user_id,relationship_id:r.relationship_id,source_intel_id:r.source_intel_id,oracle_id:r.oracle_id,target_card_name:r.target_card_name,corroborating_signal_count:new Set(valid.map(x=>x.intel_id)).size,independent_source_count:n,independent_source_type_count:t,independent_creator_count:creators.size,independent_nonvideo_source_count:nonvideo.size,corroborating_sources:sources,latest_corroboration_at:latest,convergence_score:score,convergence_state:state,refreshed_at:new Date().toISOString()}});
      refreshed++;totalSignals+=new Set(valid.map(x=>x.intel_id)).size;if(state==='multi_source')multiSource++;if(state==='strong_convergence')strong++;details.push({card:r.target_card_name,state,score,independent_sources:n,signals:new Set(valid.map(x=>x.intel_id)).size,sources});
    }
    return J({ok:true,relationships_refreshed:refreshed,corroborating_signals:totalSignals,multi_source:multiSource,strong_convergence:strong,details});
