...
Deno.serve(async req=>{
  ...
  try{
    ...
    for(const r of rels||[]){
      ...
      const links=await rest(`market_intel_scout_signal_links?select=intel_id&user_id=eq.${enc(r.user_id)}&oracle_id=eq.${enc(r.oracle_id)}&limit=1000`).catch(()=>[]);
      const ids=[...new Set((links||[]).map((x:any)=>x.intel_id).filter((x:any)=>x&&x!==r.source_intel_id))];
      const items:any[]=[];
      for(let i=0;i<ids.length;i+=30){
        const q=ids.slice(i,i+30).join(',');
        if(!q)continue;
        const rows=await rest(`market_intel_items?select=intel_id,source_name,source_type,source_profile,source_subtype,published_at,observed_at,created_at&user_id=eq.${enc(r.user_id)}&intel_id=in.(${enc(q).replaceAll('%2C',',')})&limit=1000`).catch(()=>[]);
        items.push(...(rows||[]));
      }
      ...
      const sources=[...new Set(valid.map(x=>x.source_name).filter(Boolean))];const signalCount=new Set(valid.map(x=>x.intel_id)).size;
      await rest('market_intel_synergy_convergence_cache',{method:'POST',prefer:'resolution=merge-duplicates,return=minimal',body:{user_id:r.user_id,relationship_id:r.relationship_id,source_intel_id:r.source_intel_id,oracle_id:r.oracle_id,target_card_name:r.target_card_name,corroborating_signal_count:signalCount,independent_source_count:n,independent_source_type_count:t,independent_creator_count:creators.size,independent_nonvideo_source_count:nonvideo.size,corroborating_sources:sources,latest_corroboration_at:latest,convergence_score:score,convergence_state:state,refreshed_at:new Date().toISOString()}});
