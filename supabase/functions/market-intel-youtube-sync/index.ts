...
async function fetchFeed(channelId:string){
  ...
  let lastStatus=0;
  for(const delay of [0,1200,4000,9000]){
    if(delay)await sleep(delay);
    try{
      const r=await fetch(url,{headers:{'User-Agent':'collectish-signals/1.0','Accept':'application/atom+xml,application/xml,text/xml;q=0.9,*/*;q=0.5'},redirect:'follow'});
      lastStatus=r.status;if(!r.ok)continue;
      const xml=await r.text();const items=[...xml.matchAll(/<entry>([\s\S]*?)<\/entry>/gi)].map(m=>m[1]).map(e=>{const video_id=ytId(e);const title=tag(e,'title');return{video_id,title,url:attr(e,'link','href')||`https://www.youtube.com/watch?v=${video_id}`,published_at:tag(e,'published')||null,channel_name:tag(tag(e,'author'),'name')||null}}).filter(x=>x.video_id&&x.title);
      if(items.length)return items;
    }catch{}
  }
  throw Error(`YouTube RSS ${lastStatus||'unavailable'} after retries`);
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    outer: for(const sub of enabled){
      ...
      try{
        ...
        for(const item of items){
          ...
          const prior=await rest(`source_captures?select=capture_id,metadata_json&user_id=eq.${encodeURIComponent(owner)}&capture_type=eq.video_transcript&source_key=eq.${encodeURIComponent(sourceKey)}&limit=1`).catch(()=>[]),row=prior?.[0];
    ...
    return J({ok:true,subscriptions:enabled.length,max_transcripts:maxTranscripts,transcript_attempts:transcriptAttempts,billable_credits_observed:billableCredits,saved,duplicates,events_saved:eventsSaved,unavailable_native:unavailable,failed,skipped,policy:{transcript_mode:'native',ai_generation:false,per_run_cap:maxTranscripts,rss_retry_count:4,native_retry_hours:72},report});
