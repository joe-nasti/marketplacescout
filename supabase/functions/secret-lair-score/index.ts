...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const net=adjusted*.75,evs=evScore(acquisition,net),confidence=Math.min(.9,.35+coverage*.30+pricedCoverage*.20+(evidence.length?Math.min(.9,.45+evidence.length*.015)*.15:0));
    let opportunity=weighted([[cardsScore,.20],[treatment,.14],[audience,.14],[supply,.14],[evs,.22],[liquidity,.16]]);opportunity=clamp((opportunity-concentration*.08-compression*.08)*(.72+.28*confidence));const rec=recommendation(opportunity,collector,confidence);
    const inserted=await rest(t,'secret_lair_evaluations',{method:'POST',prefer:'return=representation',body:[{release_id:drop.release_id,drop_id:drop.drop_id,evaluation_phase:'pre_sale',evaluation_status:'scored',region,finish,cards_score:money(cardsScore),treatment_score:money(treatment),audience_score:money(audience),supply_score:supply,anchor_strength:money(anchor),playable_depth:money(depth),bling_gap:money(bling),premium_competition_penalty:money(resolved.reduce((a:number,v:any)=>a+v.competition,0)/resolved.length),reprint_compression_penalty:money(compression),value_concentration_risk:money(concentration),collector_score:money(collector),opportunity_score:money(opportunity),confidence:Number(confidence.toFixed(3)),naive_comparable_ev:money(naive),compression_adjusted_ev:money(adjusted),settled_ev:money(adjusted),acquisition_cost:money(acquisition),expected_net_after_fees:money(net),expected_roi_pct:money((net-acquisition)/acquisition*100),recommendation:rec,thesis:`${finish} ${region}: ${rec.replaceAll('_',' ')}. Compression-adjusted card EV ${money(adjusted)} vs ${money(acquisition)} acquisition; collector ${money(collector)}, opportunity ${money(opportunity)}.`,what_changes_grade:'Launch sell-through by storefront, confirmed allocation evidence, realized treatment premiums, and actual TCG sales.',model_version:'secret-lair-v1-scored',score_components:{fee_rate:.25,coverage,priced_coverage:pricedCoverage,evidence_rows:evidence.length,supply_semantics:'global_supply_regional_allocation'}}]});
