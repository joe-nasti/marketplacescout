...
test('composite sealed detail rolls child products through economics and optimization',async({page})=>{
  ...
  await page.route('**/rest/v1/**',route=>{
    ...
    else if(u.includes('sealed_product_executable_ev_cache'))body=[{sealed_uuid:'gift',tcg_low_ev:87.14,direct_first_net_ev:83.2,collectish_live_out_ev:116.77,fixed_tcg_low_ev:6,fixed_collectish_live_out_ev:20.65,modeled_child_units:10,price_coverage_pct:94.2,valuation_basis:'children_plus_fixed_current_only',model_version:'hobbit-v2',valuation_as_of:'2026-09-02T12:00:00Z'},{sealed_uuid:'play',tcg_low_ev:3.46,direct_first_net_ev:3.33,collectish_live_out_ev:5.68,valuation_basis:'randomized_current_only'},{sealed_uuid:'collector',tcg_low_ev:50,direct_first_net_ev:47,collectish_live_out_ev:45,valuation_basis:'randomized_current_only'}];
  ...
  await giftRow.click();
  await expect(page.getByText('Gross reference EV',{exact:true})).toBeVisible();
  await expect(page.locator('.cx-sealed-grid')).toContainText('$136.24');
  await expect(page.locator('.cx-sealed-provenance')).toContainText('TCG Market; comparison only');
  await expect(page.locator('.cx-sealed-provenance')).toContainText('94%');
  await expect(page.locator('.cx-sealed-provenance')).toContainText('hobbit-v2');
