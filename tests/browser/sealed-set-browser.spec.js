...
test('composite sealed detail rolls child products through economics and optimization',async({page})=>{
  ...
  await page.route('**/rest/v1/**',route=>{
    ...
    else if(u.includes('sealed_product_executable_ev_cache'))body=[{sealed_uuid:'gift',tcg_low_ev:87.14,direct_first_net_ev:83.2,collectish_live_out_ev:116.77,practical_liquidation_ev:79.65,practical_median_estimate:58,practical_p10_estimate:31,top10_practical_ev_share_pct:42,practical_scout_score:61,practical_scout_grade:'D',practical_action:'KEEP SEALED',fixed_tcg_low_ev:6,fixed_collectish_live_out_ev:20.65,modeled_child_units:10,valuation_basis:'children_plus_fixed_current_only'},{sealed_uuid:'play',tcg_low_ev:3.46,direct_first_net_ev:3.33,collectish_live_out_ev:5.68,practical_liquidation_ev:3,valuation_basis:'randomized_current_only'},{sealed_uuid:'collector',tcg_low_ev:50,direct_first_net_ev:47,collectish_live_out_ev:45,practical_liquidation_ev:32,valuation_basis:'randomized_current_only'}];
  ...
  await expect(giftRow).toContainText('Practical EV');
  await expect(giftRow).toContainText('$79.65');
  await expect(giftRow).toContainText('Practical spread');
