...
(()=>{
  ...
  window.__collectishConsolidated={version:'0.7.5',builtAt:'2026-08-13T21:56:45.730Z'};
})();
...
(() => {
  ...
  async function androidHeartbeat(){
    ...
    const job=Array.isArray(claimed)?claimed[0]:null;if(!job)return;
    const progress={stage:'completed',percent:100,detail:'Authenticated TCGplayer session confirmed on Android',updatedAt:now};
    await rest(`collector_jobs?job_id=eq.${encodeURIComponent(job.job_id)}`,{method:'PATCH',body:{status:'completed',completed_at:now,lease_expires_at:null,progress_json:progress,error_message:null},prefer:'return=minimal'});
  ...
  setInterval(()=>androidHeartbeat().catch(()=>{}),30000);
  setTimeout(()=>androidHeartbeat().catch(()=>{}),2500);
})();
