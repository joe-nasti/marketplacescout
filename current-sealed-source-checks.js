...
(() => {
  ...
  function trustedCk(p){return p?.source==='cardkingdom_public'&&p?.raw_json?.matchGuard==='standalone-v5'&&['high','medium'].includes(p?.raw_json?.matchConfidence)}
  function block(tcg,ck){
    ...
    return `<section class="cx-v5-section cx-sealed-source-check"><div class="cx-section-title">Sealed source checks</div><div class="cx-sealed-grid"><div class="cx-sealed-stat"><span>TCG Low + shipping</span><strong>${money(tcg?.low_with_shipping)}</strong><small>${esc(tcg?.product_name||'No trusted TCG match')}</small></div><div class="cx-sealed-stat"><span>TCG Market</span><strong>${money(tcg?.market_price)}</strong><small>identity ${esc(tq)} · ${esc(tm)}</small></div><div class="cx-sealed-stat"><span>Card Kingdom retail</span><strong>${money(ck?.market_price)}</strong><small>${esc(ck?.product_name||'secondary match pending')}</small></div><div class="cx-sealed-stat"><span>CK status</span><strong>${ck?esc(stock.replaceAll('_',' ')):'—'}</strong><small>${ck?`match ${esc(cq)} · reference only`:'not used in score'}</small></div></div></section>`;
  }
