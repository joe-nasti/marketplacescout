...
class ReadOnlyProbeBridge(
    ...
    /** Callback-only interface attached to the authenticated Seller WebView. */
    ...
    fun sellerCallbackInterface(): SellerCallback = SellerCallback()
    ...
    private fun runFetch(url: String, method: String, body: String, mode: String, token: String) {
        ...
        // Android WebView evaluateJavascript does not await a Promise result. The
        // request therefore completes through the callback-only native interface.
        val script = """
            (function(){
                ...
                for(let attempt=1;attempt<=maxAttempts;attempt++){
                  try {
                    ...
                    if(response.ok && !loginHtml){
                      let parsed=null;
                      if(mode==='fetch_json'){try{parsed=JSON.parse(text)}catch(e){}}
                      send({ok:true,status:response.status,statusText:response.statusText,url:response.url||url,method,
                        contentType:response.headers.get('content-type')||'',elapsedMs:Date.now()-started,attempt,
                        body:parsed===null?text:parsed,checkedAt:new Date().toISOString()});
                      return;
                    }
                    ...
                    if(attempt>=maxAttempts){send({error:String(e),url,method,checkedAt:new Date().toISOString()});return;}
            ...
            val script = """
                (function(){
                  try {
                    ...
                    return JSON.stringify({title:document.title||'',url:location.href,path:location.pathname||'/',tables,links,resources,
                      bodyText:clean(document.body?.innerText||'').slice(0,${ReadOnlyProbePolicy.maxBodyChars}),checkedAt:new Date().toISOString()});
                  } catch(e) {return JSON.stringify({error:String(e),checkedAt:new Date().toISOString()});}
                })();
            """.trimIndent()
    ...
    private fun decodeJsString(raw: String): String {
        ...
        return try { JSONArray("[$raw]").getString(0) }
        catch (_: Exception) { raw.replace("\\\"", "\"").replace("\\n", "\n").replace("\\\\", "\\").trim('"') }
    }
}
