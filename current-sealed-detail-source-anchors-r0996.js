// Scout Sealed source anchors r0996 — reliable same-WebView navigation on Android.
(() => {
  const cache=new Map();
  ...
  function wrap(el,url,title,{always=false}={}){
    ...
    // Do not use target=_blank here: Android WebView can silently ignore new-window requests.
  ...
  async function decorate(){
    ...
    // Renderer can replace detail HTML for the same UUID, so never use a UUID-level early-return guard.
