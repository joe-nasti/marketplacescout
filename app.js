...
async function queueNew(){const opt=$("newSet").selectedOptions[0];if(!opt?.value){$("newScanMsg").textContent="Select a set.";return}const p={setSlug:opt.value,setName:opt.dataset.name||opt.textContent,printing:$("newPrinting").value,condition:$("newCondition").value,language:$("newLanguage").value,salesEnrich:Number($("newEnrich").value)};await queue(p);$("newScanMsg").textContent=`Queued ${p.setName}.`}
...
async function load(){
 const [scans,dev,cmd,sets]=await Promise.all([rest("marketplace_scans?select=*&order=captured_at.desc&limit=100"),rest("marketplace_devices?select=*&order=last_seen_at.desc&limit=5"),rest("marketplace_scan_commands?select=*&order=requested_at.desc&limit=20"),rest("marketplace_set_catalog?select=*&order=set_name.asc")]);scansCache=scans;
 const d=dev[0],on=d&&Date.now()-new Date(d.last_seen_at).getTime()<300000;$("device").innerHTML=d?`<b>${on?"Online":"Offline"}</b> • ${d.device_name||"PC"} • ${dt(d.last_seen_at)}`:"No heartbeat yet";
 ...
 const ps=new Map();for(const s of scans){const k=[s.set_slug,s.printing,s.condition,s.language].join("|");if(!ps.has(k))ps.set(k,s)}
 ...
 $("analyticsProfile").innerHTML=[...ps.entries()].map(([k,s])=>`<option value="${k}">${s.set_name} • ${s.printing}/${s.condition}/${s.language}</option>`).join("");if(ps.size)await analytics();
...
$("signIn").onclick=login;$("refresh").onclick=load;$("signOut").onclick=()=>{save(null);boot()};$("queueNew").onclick=queueNew;$("analyticsProfile").onchange=analytics;boot();
