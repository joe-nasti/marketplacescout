// Collectish Marketplace Scout web v0.4.1 — product shell stability fix
(() => {
  const el=id=>document.getElementById(id);
  const badge=el("appVersion"); if(badge) badge.textContent="web v0.4.1";
  ...
  function updateSummary(){
    ...
    if(!hero&&!fresh)return;
    const latest=(typeof scansCache!=="undefined"&&Array.isArray(scansCache)&&scansCache.length)?[...scansCache].sort((a,b)=>new Date(b.captured_at)-new Date(a.captured_at))[0]:null;
    if(hero){
      let hot=0,watch=0;
      const cards=el("signalCards");
      if(cards){const txt=cards.textContent||"";const hm=txt.match(/HOT\s*(\d+)/i),wm=txt.match(/WATCH\s*(\d+)/i);hot=Number(hm?.[1]||0);watch=Number(wm?.[1]||0)}
    ...
    if(fresh){
      const next=`<span>Marketplace ${latest?.captured_at?new Date(latest.captured_at).toLocaleString():"not loaded"}</span><span>EDHREC currently PC-local</span>`;
      if(fresh.innerHTML!==next)fresh.innerHTML=next;
    }
  }
  ...
  function init(){
    ...
    summaryTimer=setInterval(()=>{if(!document.hidden)updateSummary()},5000);
