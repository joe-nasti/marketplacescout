...
function readMetrics(){
  try{return JSON.parse(sessionStorage.getItem(METRIC_KEY)||'{}')}catch{return {}}
}
...
async function request(path,options,token){
  const r=await fetch(`${collectishConfig.supabaseUrl}/rest/v1/${path}`,{
    method:options.method||'GET',
    headers:headers(token,options.prefer),
...
async function baseRest(path,options={}){
  ...
  if(!out.r.ok){
    if(isJwtProblem(out.r.status,out.data,out.text)){
      document.dispatchEvent(new CustomEvent('collectish:auth-invalid'));
    }
    throw new Error(out.data?.message||out.data?.msg||`HTTP ${out.r.status}`);
  }
  return out.data;
}

function isTimeout(error){
  return String(error?.message||error||'').toLowerCase().includes('statement timeout');
}
...
export async function rest(path,options={}){
  ...
  try{
    ...
    if(method!=='GET'||!isTimeout(error))throw error;
