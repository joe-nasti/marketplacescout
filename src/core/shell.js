...
export const WEB_VERSION='0.10.23';
...
function desktopAsk(){return '<section class="cx-nav-group cx-nav-assistant"><div class="cx-nav-group-label"><strong>Assistant</strong><small>Ask across Collectish.</small></div><div class="cx-nav-sub"><button type="button" data-cx-ask-nav><span aria-hidden="true">✦</span> Ask</button></div></section>'}
...
function mobileAsk(){return '<button type="button" data-cx-ask-nav><span>Ask</span></button>'}
...
function routeFromIntentTarget(target){const page=target?.closest?.('[data-cx-page]');if(page)return page.dataset.cxPage;const group=target?.closest?.('[data-cx-group-nav]')?.dataset.cxGroupNav;if(!group)return null;return groupLastRoute[group]||GROUPS[group]?.home||null}
function prefetchIntent(event){const page=routeFromIntentTarget(event.target);if(page)window.CollectishLazyDataPages?.prefetch?.(page)}
function mobileUtilityOffset(){if(!matchMedia('(max-width:700px)').matches)return 0;return Math.ceil(document.getElementById('cxMobileUtilities')?.getBoundingClientRect().height||0)}
function scrollPageToOrigin({behavior='auto'}={}){window.scrollTo({top:mobileUtilityOffset(),behavior})}
