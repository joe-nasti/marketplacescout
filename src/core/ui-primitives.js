...
export function uiEvidenceMarker(kind='unmeasured',help=''){
  ...
  return `<span role="button" tabindex="0" class="cx-ui-evidence ${esc(d.tone)}" data-cx-evidence-kind="${esc(kind)}" data-cx-evidence-help="${esc(text)}" aria-label="${esc(`${d.label}: ${text}`)}" title="${esc(text)}">${esc(d.symbol)}</span>`;
}
...
function activateEvidence(e){
  const marker=e.target.closest?.('[data-cx-evidence-kind]');
  if(!marker)return;
  if(e.type==='keydown'&&!['Enter',' '].includes(e.key))return;
  e.preventDefault();e.stopPropagation();showEvidenceHelp(marker);
}
...
.cx-ui-evidence{display:inline-flex;align-items:center;justify-content:center;padding:0 0 0 3px;font:900 10px/1 system-ui;vertical-align:middle;cursor:pointer;user-select:none}.cx-ui-evidence:focus-visible{outline:1px solid currentColor;outline-offset:2px;border-radius:3px}.cx-ui-evidence.success{color:var(--color-success)}.cx-ui-evidence.accent{color:var(--color-accent)}.cx-ui-evidence.warning{color:var(--color-warning)}.cx-ui-evidence.danger{color:var(--color-danger)}.cx-ui-evidence.muted{color:var(--color-text-secondary)}.cx-ui-evidence-popover{position:fixed;z-index:100000;padding:10px 11px;border:1px solid var(--color-border);border-radius:10px;background:var(--color-bg-surface);box-shadow:0 10px 30px rgb(0 0 0 / .28);color:var(--color-text-primary);font-size:11px}.cx-ui-evidence-popover>div{display:flex;align-items:center;gap:6px}.cx-ui-evidence-popover p{margin:6px 0}.cx-ui-evidence-popover small{color:var(--color-text-secondary);font-size:9px}
...
`;
  ...
  document.addEventListener('click',activateEvidence);
  document.addEventListener('keydown',activateEvidence);
}
