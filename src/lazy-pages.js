// Vite-managed lazy feature chunks. Dynamic imports become content-hashed chunks.
...
function showLoading(page){
  const h=host(page);
  if(!h||h.dataset.cxLazyReady==='1')return;
...
function showError(page,err){
  ...
  h.innerHTML=`<div class="cx-page-head"><div><h2>${title(page)}</h2></div></div><div class="cx-card"><div class="cx-empty">Could not load ${title(page)}${err?`: ${String(err.message||err)}`:''}. Tap the tab to try again.</div></div>`;
}
...
document.addEventListener('click',e=>{
  ...
  if(!b)return;
  if(pageLoaders[b.dataset.cxPage])loadPage(b.dataset.cxPage).catch(()=>{});
},true);

document.addEventListener('collectish:ready',()=>queueMicrotask(()=>{
  const id=document.querySelector('.cx-page.active')?.id||'';
  if(id==='cxSealed')loadPage('sealed').catch(()=>{});
  else if(id==='cxSeller')loadPage('seller').catch(()=>{});
  else if(id==='cxSyp')loadPage('syp').catch(()=>{});
}));
