...
import { bestExitQuoteFromScoutRow, normalizeAcquisitionQuote, SOURCING_CHANNELS } from './sourcing.js';
...
function localAcquisition(row,sources){
  const observed=Number(row.cheapest_buy||row.tcg_low||sources?.[0]?.direct_buy_price||0);
  if(!Number.isFinite(observed)||observed<=0)return normalizeAcquisitionQuote({channel:SOURCING_CHANNELS.TCGPLAYER});
  return normalizeAcquisitionQuote({channel:SOURCING_CHANNELS.TCGPLAYER,unitPrice:observed,landedUnitCost:observed,sourceLabel:row.cheapest_source||'best observed exact-printing acquisition',mappingConfidence:'local_exact',capturedAt:row.refreshed_at});
}
...
async function decorate(event){
  const my=++seq,row=selectedRow(event),h=detail();if(!row||!h)return;
  h.querySelector('.cx-sealed-compare')?.remove();
  let sources=[];try{sources=await rest(identityPath(row))}catch(e){console.warn('[scout sealed compare]',e);return}
  if(my!==seq||!sources?.length)return;
  const acquisition=localAcquisition(row,sources),directBuy=Number(acquisition.landedUnitCost||0);
  const best=[...sources].filter(x=>Number(x.ev_allocated_acquisition_per_copy)>0).sort((a,b)=>{const aa=directBuy>0?100*(directBuy-Number(a.ev_allocated_acquisition_per_copy))/directBuy:-999,bb=directBuy>0?100*(directBuy-Number(b.ev_allocated_acquisition_per_copy))/directBuy:-999;return bb-aa})[0]||sources[0];
  const alloc=Number(best.ev_allocated_acquisition_per_copy),adv=directBuy>0&&alloc>0?100*(directBuy-alloc)/directBuy:null,exit=bestExitQuoteFromScoutRow(row),exitName=exit?.label||'Unmeasured',exitNet=exit?.net??null,profit=exitNet!=null&&alloc>0?exitNet-alloc:null;
  const box=document.createElement('section');box.className='cx-v5-section cx-sealed-compare';box.innerHTML=`<div class="cx-section-title">Buy direct · Crack sealed · Best exit</div><div class="cx-sealed-compare-grid"><div class="cx-sealed-compare-card"><span>Buy directly</span><b>${money(directBuy)}</b><small>${esc(acquisition.sourceLabel||'best observed exact-printing acquisition')}</small></div><div class="cx-sealed-compare-card ${adv>0?'positive':''}"><span>Best sealed route</span><b>${money(alloc)}</b><small>${esc(best.product_name||'')} · ${pct(adv)} vs direct</small></div><div class="cx-sealed-compare-card"><span>Best live exit</span><b>${money(exitNet)}</b><small>${esc(exitName)}${profit==null?'':` · ${money(profit)} vs allocated crack cost`}</small></div></div><div class="cx-sealed-source-list">${sourceRows(sources,directBuy)}</div><div class="cx-sealed-source-note">Acquisition and exit lanes now use the shared Scout Sourcing contract. Best live exit evaluates current Collectish lanes: Direct net, TCG regular net, CK cash and ManaPool net estimate. Allocated crack cost credits the expected value of the other cards opened. Pulls remain probabilistic unless fixed.</div>`;
