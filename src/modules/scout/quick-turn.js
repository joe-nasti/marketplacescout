...
function render(){
  ...
  panel.innerHTML=`<div class="cx-page-head"><div><div class="cx-section-title">Quick-turn opportunities</div><p class="cx-sub">Fast-selling cards where the estimated Direct ROI already clears the lower margin hurdle justified by liquidity. This is an execution lens, not a replacement for the base Scout thesis.</p></div><button type="button" class="cx-refresh" id="cxQuickTurnRefresh">Refresh</button></div><div class="cx-detail-list">${shown.length?shown.map(rowHtml).join(''):'<div class="cx-empty">No liquid opportunities currently clear their target margin.</div>'}</div>`;
