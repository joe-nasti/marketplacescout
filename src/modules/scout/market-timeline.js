...
function eventRow(event){
  ...
  if(event.kind==='sales'&&data.quantity_sold!=null)quick=`${Number(data.quantity_sold).toLocaleString()} sold`;
    ...
    const parts=[];
    if(data.units_from!=null&&data.units_to!=null)parts.push(`Market ${Number(data.units_from).toLocaleString()} → ${Number(data.units_to).toLocaleString()}`);
    if(data.direct_units_from!=null&&data.direct_units_to!=null&&data.direct_unit_delta)parts.push(`Direct ${Number(data.direct_units_from).toLocaleString()} → ${Number(data.direct_units_to).toLocaleString()}`);
    if(data.listings_from!=null&&data.listings_to!=null)parts.push(`${Number(data.listings_from).toLocaleString()} → ${Number(data.listings_to).toLocaleString()} listings`);
    quick=parts.join(' · ');
...
function render(data,days){
  ...
  const covered=[coverage.tcg_price_history&&'price',coverage.tcg_sales_history&&'sales',coverage.market_supply_history&&'supply',coverage.cardkingdom_depth&&'CK',coverage.syp&&'SYP',coverage.scout_score_history&&'Scout',coverage.signals&&'signals'].filter(Boolean);
  ...
  const supply=data?.supply_history||{};
  const supplyNote=coverage.market_supply_history?`<small class="cx-market-timeline-note">Supply events use COMPLETE exact-SKU marketplace snapshots. Direct drain/restock requires corroborating total-market movement; ${Number(supply.suppressed_direct_discontinuities||0).toLocaleString()} uncorroborated Direct classification change${Number(supply.suppressed_direct_discontinuities||0)===1?' was':'s were'} suppressed in this window.</small>`:'';
...
async function load(row,days=activeDays){
  ...
  try{
    const data=await rest('rpc/ask_collectish_market_timeline_v3',{method:'POST',body:{p_product_id:String(row.product_id),p_sku_id:String(row.sku_id),p_days:days}});
