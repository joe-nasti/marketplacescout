...
function compactHeader(h,row){
  ...
  const fromCard=new URL(location.href).searchParams.get('fromCard')||'',nav=document.createElement('div');nav.className='cx-card-detail-nav';nav.dataset.level=fromCard?'printing':'card';const close=h.querySelector('.cx-mobile-detail-close');if(close){close.textContent='Back';close.setAttribute('aria-label',fromCard?'Back to Card':'Back to Scout results');nav.append(close)}const trail=document.createElement('div');trail.innerHTML=fromCard?`<strong>Card · Printing</strong><small>${row.set_name||'Unknown set'}${row.collector_number?` · #${row.collector_number}`:''} · exact SKU</small>`:`<strong>Scout · ${position||1} of ${visible.length||1}</strong><small>${score(row)>=80?'HOT · ':''}score ↓ · filters ${filterCount}</small>`;nav.append(trail);h.prepend(nav);
