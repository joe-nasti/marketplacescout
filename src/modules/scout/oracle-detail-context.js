let installed=false,familyRows=[],familyOracle='',openedSku='',decorateSeq=0;
...
function awardExplanations(row){
  ...
  for(const [key,label,dir,fmt] of specs){
    ...
    out.push({label,reason,next:next?`Next best: ${fmt(next.v)} · ${next.r.set_code||next.r.set_name||'printing'} ${next.r.collector_number?`#${next.r.collector_number}`:''}`.trim():''});
  }
  return out;
}
function ensureStyle(){
  if(document.getElementById('cxOracleDetailContextStyle'))return;
  const s=document.createElement('style');s.id='cxOracleDetailContextStyle';s.textContent=`.cx-oracle-detail-back{order:-1}.cx-oracle-win-explain{margin:10px 0;padding:10px;border:1px solid var(--cx-border,#2a3440);border-radius:10px;background:rgba(45,127,249,.06)}.cx-oracle-win-explain>strong{display:block;font-size:12px;margin-bottom:6px}.cx-oracle-win-item{padding:6px 0;border-top:1px solid rgba(127,127,127,.12)}.cx-oracle-win-item:first-of-type{border-top:0;padding-top:0}.cx-oracle-win-item b{font-size:10px;color:var(--cx-accent,#5aa2ff)}.cx-oracle-win-item span,.cx-oracle-win-item small{display:block;font-size:11px;margin-top:2px}.cx-oracle-win-item small{opacity:.65}`;document.head.appendChild(s);
}
...
function decorateDetail(sku,token,attempt=0){
  ...
  const panel=document.createElement('div');panel.className='cx-oracle-win-explain';panel.innerHTML=`<strong>Why this printing wins</strong>${wins.map(w=>`<div class="cx-oracle-win-item"><b>${esc(w.label)}</b><span>${esc(w.reason)}</span>${w.next?`<small>${esc(w.next)}</small>`:''}</div>`).join('')}`;
...
function clearContext(){if(new URL(location.href).searchParams.get('oracle'))return;familyRows=[];familyOracle='';openedSku='';decorateSeq++}
export function installOracleDetailContext(){
  if(installed)return;installed=true;ensureStyle();document.addEventListener('collectish:scout-universal-results',onFamilyResults);document.addEventListener('click',onClick);document.addEventListener('collectish:page-change',clearContext);
}
