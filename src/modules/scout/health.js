...
const WARN_MIN=75;
const BAD_MIN=105;
const OFFICIAL_PRICE_WARN_MIN=5;
const TCGCSV_FALLBACK_WARN_MIN=30*60;
...
function clockText(row,{successAt=row?.at||null,cadence=''}={}){const status=String(row?.status||'').toLowerCase(),attemptAt=row?.attemptAt||row?.last_started_at||null,suffix=cadence?` · ${cadence}`:'';if(row?.last_error)return`Error ${rel(attemptAt)} · last good ${rel(successAt)}${suffix}`;if(status==='failed')return`Failed ${rel(attemptAt)} · last good ${rel(successAt)}${suffix}`;if(status==='paused')return`Paused · last good ${rel(successAt)}${suffix}`;if(!successAt)return`Never completed${suffix}`;return`${rel(successAt)}${suffix}`}
export async function readScoutHealth(){
  const [cacheRows,stateRows,officialRows,fallbackRows]=await Promise.all([
    ...
    rest('tcgplayer_official_sync_state?select=scope,last_started_at,last_completed_at,last_requested,last_written,last_error,updated_at&scope=eq.scout&limit=1').catch(()=>[]),
  ...
  const priceAt=official.last_completed_at||null;
  const priceMins=ageMin(priceAt);
  const priceState=official.last_error||!priceAt||priceMins>OFFICIAL_PRICE_WARN_MIN?'warn':'good';
  const fallbackAt=fallback.source_updated_at||fallback.last_completed_at||null;
  const fallbackMins=ageMin(fallbackAt);
  const fallbackState=fallback.status==='failed'||fallback.status==='paused'||!fallbackAt||fallbackMins>TCGCSV_FALLBACK_WARN_MIN?'warn':'good';
  return{cacheAt,mins,state,primary,watchdog,price:{...official,priceAt,at:priceAt,attemptAt:official.last_started_at||null,mins:priceMins,state:priceState,source:'tcgplayer_official'},priceFallback:{...fallback,at:fallbackAt,attemptAt:fallback.last_started_at||null,mins:fallbackMins,state:fallbackState,source:'tcgcsv'},sales,volatility};
}
...
function renderFreshnessStrip(x){
  ...
  const fallback=x.priceFallback?.at?` · fallback TCGCSV ${rel(x.priceFallback.at)}`:'';
  const items=[`<span style="color:${scoreTone}"><strong>Scout score</strong> · ${esc(rel(x.cacheAt))}${x.state==='bad'?' · STALE':''}</span>`,item('TCGplayer prices',x.price,`${clockText(x.price,{successAt:x.price?.priceAt,cadence:'official API'})}${fallback}`),item('Sales',x.sales,clockText(x.sales,{cadence:'3h cadence'})),item('Volatility',x.volatility,clockText(x.volatility,{cadence:'6h cadence'}))];
...
function renderAdminCard(x){const grid=document.getElementById('cxAdminOverviewCards');if(!grid)return;let card=document.getElementById('cxAdminScoutHealth');if(!card){card=document.createElement('div');card.id='cxAdminScoutHealth';grid.prepend(card)}card.className=`cx-admin-summary-card ${x.state}`;const detail=x.primary?.detail||{},durations=detail.durations_ms||{},failed=detail.failed_phase?` · failed ${detail.failed_phase}`:'';const fallback=x.priceFallback?.at?` · TCGCSV fallback ${rel(x.priceFallback.at)}`:'';card.innerHTML=`<span>Scout freshness</span><strong>${x.state==='good'?'Healthy':x.state==='warn'?'Score aging':'SCORE STALE'}</strong><small>Score ${esc(rel(x.cacheAt))}${esc(failed)}<br>TCGplayer API ${esc(clockText(x.price,{successAt:x.price?.priceAt,cadence:'primary'}))}${esc(fallback)}<br>Sales ${esc(clockText(x.sales,{cadence:'every 3h'}))}<br>Volatility ${esc(clockText(x.volatility,{cadence:'every 6h'}))}<br>agg ${esc(secs(durations['24h_aggregation']))} · v5 ${esc(secs(durations['v5_shadow']))} · cache ${esc(secs(durations['promoted_cache']))}</small>`;const overall=document.getElementById('cxAdminOverallState');if(overall&&(x.state==='bad'||x.price?.state==='warn'||x.sales?.state==='warn'||x.volatility?.state==='warn')){overall.textContent='ATTENTION';overall.className='cx-admin-console-state attention'}}
