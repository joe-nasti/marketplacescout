...
const pipe=(row,warn)=>{const at=row?.last_completed_at||null,mins=ageMin(at),bad=row?.status==='failed'||row?.status==='paused'||!at||mins>warn;return{...(row||{}),at,mins,state:bad?'warn':'good'}};
export async function readScoutHealth(){
  ...
  const priceState=price.status==='failed'||!priceAt||priceMins>PRICE_WARN_MIN?'warn':'good';
  return{cacheAt,mins,state,primary,watchdog,price:{...price,priceAt,mins:priceMins,state:priceState},sales,volatility};
}
function chip(label,at,state,suffix=''){const tone=state==='warn'?'#9a6700':'inherit';return`<span style="color:${tone}"><strong>${esc(label)}</strong> · ${esc(rel(at))}${suffix}</span>`}
function renderFreshnessStrip(x){
  ...
  const items=[`<span style="color:${scoreTone}"><strong>Scout score</strong> · ${esc(rel(x.cacheAt))}${x.state==='bad'?' · STALE':''}</span>`,chip('TCGplayer prices',x.price?.priceAt,x.price?.state,' · daily TCGCSV'),chip('Sales',x.sales?.at,x.sales?.state,' · 3h cadence'),chip('Volatility',x.volatility?.at,x.volatility?.state,' · 6h cadence')];
...
function renderAdminCard(x){const grid=document.getElementById('cxAdminOverviewCards');if(!grid)return;let card=document.getElementById('cxAdminScoutHealth');if(!card){card=document.createElement('div');card.id='cxAdminScoutHealth';grid.prepend(card)}card.className=`cx-admin-summary-card ${x.state}`;const detail=x.primary?.detail||{},durations=detail.durations_ms||{},failed=detail.failed_phase?` · failed ${detail.failed_phase}`:'';card.innerHTML=`<span>Scout freshness</span><strong>${x.state==='good'?'Healthy':x.state==='warn'?'Score aging':'SCORE STALE'}</strong><small>Score ${esc(rel(x.cacheAt))}${esc(failed)}<br>TCGplayer ${esc(rel(x.price?.priceAt))} · daily TCGCSV${x.price?.status==='failed'?' · FAILED':''}<br>Sales ${esc(rel(x.sales?.at))} · every 3h${x.sales?.state==='warn'?' · CHECK':''}<br>Volatility ${esc(rel(x.volatility?.at))} · every 6h${x.volatility?.state==='warn'?' · CHECK':''}<br>agg ${esc(secs(durations['24h_aggregation']))} · v5 ${esc(secs(durations['v5_shadow']))} · cache ${esc(secs(durations['promoted_cache']))}</small>`;const overall=document.getElementById('cxAdminOverallState');if(overall&&(x.state==='bad'||x.sales?.state==='warn'||x.volatility?.state==='warn')){overall.textContent='ATTENTION';overall.className='cx-admin-console-state attention'}}
