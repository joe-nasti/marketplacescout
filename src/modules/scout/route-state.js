...
function applyState(){
  ...
  const state=urlState(),{view,sku,fromCard}=state,lookup=lookupSku(sku);
  explicitSku=sku;
  applying=true;
  try{
    renderer.setSaved(view);
    if(lookup&&view!=='quick')openLookup({...state,...lookup,sku:''});
    else if(sku&&view!=='quick')window.CollectishScoutDetailNavigation?.open?.({sku_id:sku,from_card_sku:fromCard});
