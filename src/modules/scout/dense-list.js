...
function decorate(){
  ...
  for(const card of host.querySelectorAll(':scope > .cx-scout-card')){
    ...
    card.classList.remove('cx-scout-compact-card');card.classList.add('cx-scout-dense-row');
  ...
  if(section&&!section.querySelector('.cx-scout-dense-head')){const head=document.createElement('div');head.className='cx-scout-dense-head';head.innerHTML='<span>Card</span><span>Score</span><span>Market</span><span>Direct</span><span>D premium</span><span>Velocity</span><span>CK BL</span><span>Signal</span>';section.insertBefore(head,host)}
