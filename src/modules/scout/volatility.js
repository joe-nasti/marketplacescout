...
const marker=()=>uiEvidenceMarker('verified','Measured TCGplayer marketplace SKU volatility. This is not Direct-only behavior.');
...
function applyCard(card){
  ...
  badge.innerHTML=`MKT ${label(v.volatility)}${adj?` ${adj}`:''} ${marker()}`;
  badge.title=`TCGplayer marketplace SKU volatility${v.z_score==null?'':` · z ${Number(v.z_score).toFixed(2)}`}`;
  badge.classList.toggle('verify',String(v.volatility).toUpperCase()==='HIGH');
...
function applyDetail(){
  ...
  const badges=host.querySelector('.cx-v5-badges');if(badges&&!badges.querySelector('[data-volatility-detail]')){const el=document.createElement('span');el.dataset.volatilityDetail='1';el.className='cx-v5-badge';if(String(v.volatility).toUpperCase()==='HIGH')el.classList.add('verify');el.innerHTML=`MKT ${label(v.volatility)}${volatilityAdjustment(v.volatility)?' · -3 score':''} ${marker()}`;badges.append(el)}
  const details=host.querySelector('.cx-v5-details .cx-v5-grid');if(details&&!details.querySelector('[data-volatility-stat]')){const el=document.createElement('div');el.className='cx-v5-stat';el.dataset.volatilityStat='1';el.innerHTML=`<span>Marketplace volatility</span><strong>${esc(String(v.volatility||'—'))} ${marker()}</strong><small>${v.z_score==null?'TCGplayer SKU-level measurement':`z-score ${Number(v.z_score).toFixed(2)}`}</small>`;details.append(el)}
}
