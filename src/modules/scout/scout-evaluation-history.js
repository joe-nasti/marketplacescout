...
function render(data,outcomes,episodes,attribution){
  ...
  const episodeBody=episodeRows.length?episodeRows.slice(0,3).map(ep=>episodeCard(ep,attrMap.get(String(ep.episode_id)))).join(''):'';
  return `<section class="cx-v5-section cx-scout-replay">
    ...
    ${episodeBody?`<div class="cx-scout-episodes"><div class="cx-scout-episodes-title">Opportunity episodes</div>${episodeBody}<small class="cx-scout-episode-note">Pre-open evidence is association/context, not proof that a source caused the opportunity.</small></div>`:''}
...
async function decorate(event){
  ...
  const seq=++request;
  try{
    const [data,outcomes,episodes,attribution]=await Promise.all([
    ...
    const section=document.createRange().createContextualFragment(render(data,outcomes,episodes,attribution)).firstElementChild;
