...
function productIdOf(row){return String(row?.product_id||row?.productId||row?.entity?.product_id||'').trim()}
...
async function discoverProduct(row,{desiredFinish=null,force=false,reason='scout_card_open_discovery'}={}){
  const pid=productIdOf(row);if(!pid||!shouldDiscover(row,force))return null;
  if(discoveryInflight.has(pid))return discoveryInflight.get(pid);
  const task=(async()=>{try{
    const data=await invokeFunction('scout-tcgplayer-sku-discovery',{product_id:pid,...(desiredFinish?{desired_finish:desiredFinish}:{}),desired_condition:'NEAR MINT',desired_language:'ENGLISH',persist:true,force,reason});
    document.dispatchEvent(new CustomEvent('collectish:scout-sku-discovery-complete',{detail:{productId:pid,skuId:skuOf(row?.sku_id),learned:Number(data?.materialized_nm_english_count||0),data}}));
    return data;
  }catch(error){console.warn('Scout on-demand SKU discovery failed',error);document.dispatchEvent(new CustomEvent('collectish:scout-sku-discovery-failed',{detail:{productId:pid,skuId:skuOf(row?.sku_id),error:String(error?.message||error)}}));return null}finally{discoveryInflight.delete(pid)}})();
  discoveryInflight.set(pid,task);return task;
}
...
export function openScoutDetail(detail={}){
  ...
  if(!summary?.sku_id)return false;
...
function openEvent(e){
  ...
  if(!openScoutDetail(detail)&&detail?.product_id)void discoverProduct(detail);
}
