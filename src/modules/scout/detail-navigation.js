...
export function openScoutDetail(detail={}){const renderer=window.CollectishScoutRenderer;if(!renderer?.renderDetail)return false;const summary=resolveSummary(detail);if(!summary?.sku_id)return false;const sku=skuOf(summary.sku_id);store.update('scout',{selectedSku:summary.sku_id});document.querySelectorAll('#cxParityCards .cx-scout-card[data-sku]').forEach(card=>card.classList.toggle('selected',skuOf(card.dataset.sku)===sku));void renderer.renderDetail(summary,true);void prefetchAskCardContext(summary);void discoverProduct(summary);return true}
