...
function inlineText(r){if(!r)return'';return r.suggested_additional_qty>0?`BUY ${r.suggested_additional_qty} · ${money(r.suggested_capital)} · ~${days(r.expected_days_to_exit)}`:`OWN ${Number(r.existing_qty||0).toFixed(0)} · sized`}
function decorateExecutionPanels(){
  ...
  document.querySelectorAll('[data-quick-turn-sku],[data-action-sku]').forEach(el=>{
    ...
    const target=el.querySelector('span:last-child')||el;const s=document.createElement('small');s.className='cx-position-inline';s.textContent=`Position: ${inlineText(r)}`;s.title=r.sizing_reason||'Liquidity-adjusted position sizing';target.appendChild(s);
  });
}
function decorateScoutList(){
  ...
  document.querySelectorAll('#cxParityCards .cx-scout-card').forEach(card=>{
    ...
    const top=card.querySelector('.cx-scout-card-top');if(!top)return;const b=document.createElement('span');b.className='cx-v5-badge cx-position-badge';b.textContent=`BUY ${r.suggested_additional_qty}`;b.title=`Suggested additional position · ${money(r.suggested_capital)} capital · modeled ${days(r.expected_days_to_exit)} exit`;top.appendChild(b);
  });
}
...
function decorateScoutDetail(sku){
  ...
  section.innerHTML=`<div class="cx-section-title">Position sizing <span class="cx-signal-stage ${buy>=5?'leading':'confirming'}">${esc(r.sizing_class||'SIZE')}</span></div><div class="cx-v5-grid">${stat('Suggested additional',buy>0?`${buy} copies`:'No additional buy',buy>0?`${money(r.suggested_capital)} estimated capital`:'current position already sized')}${stat('Current / target',`${owned.toFixed(0)} / ${target} copies`,`${r.target_days||'—'} target days of exposure`)}${stat('Modeled exit',`~${days(r.expected_days_to_exit)}`,`${Number(r.estimated_capture_per_day||0).toFixed(2)} seller-capture/day proxy`)}${stat('Trade hurdle',`${Number(r.direct_roi_pct||0).toFixed(1)}% ROI`,`${Number(r.target_roi_pct||0).toFixed(1)}% target · +${Number(r.margin_cushion_pct||0).toFixed(1)} pts cushion`)}</div><small class="cx-sub">${esc(r.sizing_reason||'Position size is capped by estimated seller capture, liquidity, margin cushion, current inventory and per-SKU capital limits.')} This is a sizing heuristic, not a guarantee of sell-through.</small>`;
