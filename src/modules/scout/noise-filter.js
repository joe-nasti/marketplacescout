...
function ensureUi(){const page=document.getElementById('cxScout'),toolbar=page?.querySelector('.cx-scout-toolbar');if(!page||!toolbar)return null;let wrap=page.querySelector('.cx-scout-noise-filter');if(!wrap){wrap=document.createElement('div');wrap.className='cx-scout-noise-filter';wrap.innerHTML=`<label><input type="checkbox" id="cxScoutHideLowMarket"> <span>Hide Market &lt; $${LOW_MARKET.toFixed(0)} unless demand is surging</span></label><small id="cxScoutNoiseCount"></small>`;toolbar.insertAdjacentElement('afterend',wrap);const input=wrap.querySelector('#cxScoutHideLowMarket');input.addEventListener('change',()=>{setEnabled(input.checked);apply()})}const input=wrap.querySelector('#cxScoutHideLowMarket');if(input)input.checked=enabled();return wrap}
