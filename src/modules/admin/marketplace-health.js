...
(() => {
  ...
  async function load(){
    ...
    try{
      ...
      let box=admin.querySelector('.cx-marketplace-health');if(!box){box=document.createElement('div');box.className='cx-card cx-span-12 cx-marketplace-health';const singles=document.querySelector('#cxAdminConsole [data-admin-panel="singles"]'),slot=document.getElementById('cxAdminSinglesModules');(slot||singles||admin).prepend(box)}
