// Collectish Admin operations console — event-driven progressive operations UI; no startup network calls.
(() => {
  ...
  function ensure(){
    ...
    if(!shell){
      ...
      shell.innerHTML=`<div class="cx-admin-console-head"><div><h2>Operations</h2><p>Control syncs, see failures early, and verify data freshness.</p></div><span class="cx-admin-console-state" id="cxAdminOverallState">Checking…</span></div>
        ...
        <section class="cx-admin-panel" data-admin-panel="overview"><div id="cxAdminOverviewCards" class="cx-admin-summary-grid"><div class="cx-admin-loading">Loading operational health…</div></div><div id="cxAdminOverviewModules"></div></section>
  ...
  function metric(label,value,sub,state='neutral'){
    return `<div class="cx-admin-summary-card ${esc(state)}"><span>${esc(label)}</span><strong>${esc(value)}</strong>${sub?`<small>${esc(sub)}</small>`:''}</div>`;
  }
  function source(label,state,last,sub){
    return `<div class="cx-admin-source-row"><div><strong>${esc(label)}</strong><small>${esc(sub||'')}</small></div><div class="cx-admin-source-meta"><span class="cx-admin-status ${esc(state)}">${esc(state.toUpperCase())}</span><b>${esc(age(last))}</b><small>${esc(fmt(last))}</small></div></div>`;
  }
  ...
  async function load(){
    ...
    try{
      ...
      const breaker=mismatch>=2||http500>=5||(terminal.length>=6&&rate>=.40);
      const paused=(jobs||[]).filter(x=>x.status==='queued'&&x.progress_json?.pausedBy==='marketplace_circuit_breaker').length;
      ...
      const sealedHealthy=Boolean(evLast)&&Date.now()-new Date(evLast).getTime()<36*36e5;
      const overall=breaker||!sealedHealthy?'ATTENTION':'HEALTHY';
      ...
      shell.querySelector('#cxAdminOverviewCards').innerHTML=[
        metric('Singles scanning',breaker?'Paused':'Healthy',`${completed.length} completed · ${failed.length} failed in 3h`,breaker?'bad':'good'),
        metric('Enabled sets',String(enabled.length),`${due.length} currently due`,due.length>20?'warn':'neutral'),
        metric('Sealed EV',sealedHealthy?'Fresh':'Attention',evLast?`Updated ${age(evLast)}`:'No EV refresh',sealedHealthy?'good':'bad'),
        metric('Latest set scan',latestScan?age(latestScan):'Never',latestScan?fmt(latestScan):'',latestScan?'neutral':'bad')
      ].join('');
      shell.querySelector('#cxAdminSinglesSummary').innerHTML=[
        metric('Enabled',String(enabled.length),'scan profiles'),metric('Due / overdue',String(due.length),'waiting for admission',due.length?'warn':'good'),metric('Recent success',String(completed.length),'rolling 3h'),metric('Recent failure',String(failed.length),`${mismatch} filter · ${http500} HTTP 500`,failed.length?'bad':'good'),metric('Deferred',String(paused),'circuit-breaker queue',paused?'warn':'neutral')
      ].join('');
      ...
      shell.querySelector('#cxAdminSealedSources').innerHTML=[
        source('TCG sealed acquisition',tcgIso&&Date.now()-tcgLast<36*36e5?'good':'warn',tcgIso,'Authoritative sealed acquisition source'),
        source('Card Kingdom sealed reference',ckIso&&Date.now()-ckLast<72*36e5?'good':'warn',ckIso,'Reference-only sealed retail'),
        source('Component EV / scoring',sealedHealthy?'good':'warn',evLast,'Precon EV and Scout Sealed score'),
        source('Direct component coverage',directLast&&Date.now()-new Date(directLast).getTime()<36*36e5?'good':'warn',directLast,'Economically meaningful component SKUs'),
        source('SYP component coverage',sypLast?'good':'neutral',sypLast,'Stored SYP eligibility / max quantity'),
        source('MTGJSON identity + prices',syncLast?'good':'warn',syncLast,(sync||[]).map(x=>`${x.feed}: ${x.status||'unknown'}`).join(' · '))
      ].join('');
      relocate(shell);
    }catch(e){
      const ov=shell.querySelector('#cxAdminOverviewCards');if(ov)ov.innerHTML=`<div class="cx-admin-error">Couldn’t load Admin health: ${esc(e.message||e)}</div>`;
    }finally{loading=false}
  }

  document.addEventListener('click',e=>{
    if(e.target?.closest?.('[data-cx-page="admin"]'))setTimeout(()=>{ensure();load();setTimeout(()=>relocate(),180);setTimeout(()=>relocate(),600)},140);
  },true);
