...
async function render(){
  ...
  try{
    const [s,e]=await Promise.all([
      ...
      rest('store_inventory_change_events?select=detected_at,change_type,source,product_id,old_quantity,new_quantity,old_price,new_price&order=detected_at.desc&limit=5')
    ]);
    const x=s?.[0]||{},d=x.detail||{};
    el.innerHTML=`<div class="cx-section-title">Smart inventory monitor</div><div class="cx-inventory-progress-meta"><span>Last check <b>${esc(age(x.last_targeted_check_at||x.updated_at))}</b></span><span>Sales observed <b>${Number(d.salesObserved||0).toLocaleString()}</b></span><span>Products checked <b>${Number(d.productsChecked||0).toLocaleString()}</b></span><span>External sweep <b>${Number(d.sweepProductsChecked||0).toLocaleString()}</b></span><span>Changes found <b>${Number(d.changesDetected||0).toLocaleString()}</b></span><span>Full audit <b>${d.fullAuditDue?'Due':'Current'}</b></span></div>${x.last_error?`<div class="cx-inventory-error">${esc(x.last_error)}</div>`:''}<details><summary>Recent detected changes</summary><div class="cx-inventory-progress-details">${(e||[]).length?(e||[]).map(r=>`<span>${esc(fmt(r.detected_at))} · <b>${esc(r.change_type)}</b> · product ${esc(r.product_id||'—')}${r.old_quantity!=null||r.new_quantity!=null?` · qty ${esc(r.old_quantity)} → ${esc(r.new_quantity)}`:''}${r.old_price!=null||r.new_price!=null?` · price ${esc(r.old_price)} → ${esc(r.new_price)}`:''}</span>`).join(''):'<span>No changes detected yet.</span>'}</div></details>`;
