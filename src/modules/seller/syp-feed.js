...
// Collectish SYP — lifecycle-mounted, state-backed, server-paged feed.
(() => {
  const PAGE=100;
  ...
  let tab=initial.tab||'products',page=initial.page||0,sort=initial.sort||{key:'last_seen',dir:'desc'};
  let stats=initial.stats||null,opts=initial.opts||{sets:[],conditions:[],event_types:[]},installed=false,loading=false,seq=0,debounce=0;
  ...
  function shell(host){
    ...
    host.querySelectorAll('[data-syp-tab]').forEach(b=>b.onclick=()=>{
      tab=b.dataset.sypTab;page=0;sort=tab==='products'?{key:'last_seen',dir:'desc'}:{key:'changed_at',dir:'desc'};
      sync({tab,page,sort});renderBody();
    });
  }
  ...
  function productPath(limit=PAGE,offset=page*PAGE){
    ...
    s+=searchOr(q,['product_name','set_name','tcgplayer_id']);if(set)s+=`&${qp('set_name','eq.'+set)}`;if(cond)s+=`&${qp('condition','eq.'+cond)}`;
    ...
    if(first)s+=`&first_seen=gte.${encodeURIComponent(first+'T00:00:00Z')}`;if(last)s+=`&last_seen=gte.${encodeURIComponent(last+'T00:00:00Z')}`;return s;
  }
  ...
  async function loadPage(){
    ...
    try{
      ...
      wireSort();pager(rows);document.dispatchEvent(new CustomEvent('collectish:syp-page-rendered',{detail:{tab,page,count:rows.length}}));
    }catch(e){if(my===seq){sync({status:'error',error:String(e.message||e)});table.innerHTML=`<div class="cx-empty">${esc(e.message)}</div>`}}
  }
  ...
  async function load(){
    const host=document.getElementById('cxSyp');if(!host||loading)return;loading=true;sync({status:'loading'});host.dataset.sypParity='loading';host.innerHTML='<div class="cx-page-head"><div><h2>SYP</h2><p>Eligibility, quantities and product-state changes.</p></div></div><div class="cx-empty">Loading SYP…</div>';
    try{[stats,opts]=await Promise.all([rest('rpc/syp_dashboard_stats',{method:'POST',body:{}}),rest('rpc/syp_filter_options_rpc',{method:'POST',body:{}})]);sync({stats,opts,tab,page,sort,status:'ready'});shell(host);renderBody();host.dataset.sypParity='ready';document.dispatchEvent(new CustomEvent('collectish:syp-rendered',{detail:{tab}}))}catch(e){sync({status:'error',error:String(e.message||e)});host.innerHTML=`<div class="cx-empty">${esc(e.message)}</div>`}finally{loading=false}
  }
  function install(){const h=document.getElementById('cxSyp');if(!h)return false;if(installed)return true;installed=true;load();return true}
  ...
  window.CollectishSyp={load,loadPage,renderBody};
})();