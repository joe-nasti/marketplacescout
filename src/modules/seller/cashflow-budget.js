...
function confirmed(){return row?.input_status==='auto_plus_manual'||row?.input_status==='manual_only'}
function render(){
  ...
  const safe=Number(row.safe_additional_buy_budget||0),auto=Number(row.tcg_buyer_purchase_spend||0),manual=Number(row.manual_purchase_adjustment||0),orders=Number(row.buyer_orders_count||0),ready=confirmed();
  const status=row.input_status==='auto_buyer_only'?'TCG buyer spend is automatic; confirm any off-platform/operating spend below.':row.input_status==='needs_spend_input'?'No buyer orders or monthly spend inputs yet.':'Monthly spend inputs confirmed.';
  p.innerHTML=`<div class="cx-page-head"><div><div class="cx-section-title">Monthly buying budget</div><p class="cx-sub">Balances marketplace payouts received against automatic TCGplayer buyer purchases, off-platform buys, operating spend, and a reserve floor.</p></div><button type="button" class="cx-refresh" id="cxCashflowRefresh">Refresh</button></div>
  <div class="cx-v5-grid">${stat('Cash received',money(row.posted_marketplace_cash),`${money(row.pending_marketplace_cash)} pending`)}${stat('TCG buyer spend',money(auto),`${orders} imported order${orders===1?'':'s'} this month`)}${stat('Manual / off-platform',money(manual),`${money(row.operating_expense_spend)} external operating spend`)}${stat('Safe additional buys',money(safe),ready?'ready for Scout allocation':'provisional until month is confirmed')}</div>
  <div class="cx-scout-toolbar"><label class="cx-sub">Off-platform / manual buys <input id="cxCashflowInventorySpend" inputmode="decimal" type="number" min="0" step="1" value="${esc(row.manual_purchase_adjustment||0)}"></label><label class="cx-sub">Other spend not netted from payouts <input id="cxCashflowOperatingSpend" inputmode="decimal" type="number" min="0" step="1" value="${esc(row.operating_expense_spend||0)}"></label><label class="cx-sub">Reserve % <input id="cxCashflowReserve" inputmode="decimal" type="number" min="0" max="100" step="1" value="${esc(row.reserve_pct||15)}"></label><label class="cx-sub">Buy target % <input id="cxCashflowTarget" inputmode="decimal" type="number" min="0" max="150" step="1" value="${esc(row.purchase_target_pct||70)}"></label><button type="button" class="cx-primary" id="cxCashflowSave">${ready?'Update month':'Confirm month'}</button></div>
  <small class="cx-sub">${esc(status)} Marketplace fees and adjustments are already reflected in payout cash. Inventory spend used by the budget is ${money(row.inventory_purchase_spend)} total: ${money(auto)} from imported TCGplayer buyer orders plus ${money(manual)} manual/off-platform. Current rule preserves ${pct(row.reserve_pct)} of cash received and targets inventory purchases near ${pct(row.purchase_target_pct)} of the stronger of current-month cash received or the trailing 3-month payout average.</small>`;
...
document.addEventListener('collectish:buyer-orders-changed',()=>{lastLoadedAt=0;void load({force:true})});
