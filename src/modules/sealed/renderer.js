...
function calibratedReleaseSummary(ageDays,ev,rows){
  const ready=rows.filter(x=>x.calibration_status==='READY'&&Number(x.cohort_count)>=3).sort((a,b)=>Number(a.horizon_days)-Number(b.horizon_days));
  ...
  if(!cards)return'';
  return `<section class="cx-sealed-component-summary"><div class="cx-section-title">Calibrated release trajectory</div><div class="cx-sealed-grid">${cards}</div><div class="cx-sealed-econ-note">Projection applies the remaining portion of the observed Play Booster curve to current Practical EV. The curve uses matched TCGCSV Market baskets only for historical shape; current executable value remains Direct-first / TCG Low fallback. Low-confidence ranges do not change the Scout grade.</div></section>`;
}
function releaseStressSummary(row,family,calibration=[]){
  ...
  const calibrated=calibratedReleaseSummary(ageDays,ev,calibration);if(calibrated)return calibrated;
...
async function loadDetailData(r,force=false){
  ...
  return loadResource(`sealed.detail:v10:${r.sealed_uuid}`,async()=>{
    const [cards,price,contents,children,analogs,backtests,lifecycles,transitions,calibration]=await Promise.all([
