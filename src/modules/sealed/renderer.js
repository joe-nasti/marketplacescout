...
const modeledEv=r=>{const value=Number(familyEconomics(r)?.collectish_live_out_ev);return Number.isFinite(value)&&value>0?value:null};
...
function provenance(family,composite=false){
  const basis=human(family?.valuation_basis||family?.crack_value_basis||'not modeled'),coverage=family?.price_coverage_pct==null?'—':`${Number(family.price_coverage_pct).toFixed(0)}%`,version=family?.model_version||family?.adapter_model_version||'—',asOf=family?.valuation_as_of||family?.backtest_as_of,dated=asOf?new Date(asOf).toLocaleDateString(undefined,{year:'numeric',month:'short',day:'numeric'}):'—';
  return `<section class="cx-sealed-provenance"><div class="cx-section-title">EV provenance</div><dl><div><dt>Gross reference</dt><dd>Collation-weighted TCG Market; comparison only, never an exit or recommendation input.</dd></div><div><dt>TCG Low EV</dt><dd>Collation-weighted current TCG Low before seller fees.</dd></div><div><dt>Direct-first net</dt><dd>Live Direct Low after the Direct fee schedule; regular TCG Low net is the fallback when Direct is unavailable.</dd></div><div><dt>Collectish live-out</dt><dd>Best current executable net route per ${composite?'component or pull':'pull'} across Direct, regular TCGplayer, Card Kingdom cash, and ManaPool. Randomized contents exclude SYP and stale Direct.</dd></div></dl><footer><span><b>Method</b> ${esc(basis)}</span><span><b>Price coverage</b> ${esc(coverage)}</span><span><b>Model</b> ${esc(version)}</span><span><b>Valued</b> ${esc(dated)}</span></footer></section>`;
}
...
function renderProductRows(visible,selected){
  const types=setMap();
  return visible.map(r=>{const a=acquisition(r),sc=score(r),type=types.get(String(r.set_code||'').toUpperCase()),family=familyEconomics(r),ev=modeledEv(r),spread=modeledSpread(r),roi=spread!=null&&a>0?100*spread/a:null,evSub=family?.valuation_basis?human(family.valuation_basis):'executable outlets only';return `<button type="button" class="cx-sealed-row ${String(r.sealed_uuid)===String(selected)?'selected':''}" data-deck="${esc(r.sealed_uuid)}"><div class="cx-sealed-name"><strong>${esc(r.product_name)}</strong><small>${esc(human(r.subtype||r.category||type||''))} · ${esc(r.release_date||'')}</small><div class="cx-sealed-badges">${sc==null?'<span class="cx-sealed-badge risk">NOT GRADED</span>':`<span class="cx-sealed-badge direct">${esc(grade(r))} · ${sc.toFixed(1)}</span>`}${badges(r)}</div></div>${metric('Sealed buy',money(a),human(r.lifecycle_status||'catalog only'))}${metric('Collectish EV',money(ev),evSub)}${metric('TCG Low EV',money(family?.tcg_low_ev),'gross pull value','cx-sealed-hide-mobile')}${metric('Collectish spread',spread==null?'—':money(spread),roi==null?'':`${roi>=0?'+':''}${roi.toFixed(1)}%`,spread==null?'cx-sealed-pending':spread>=0?'cx-sealed-positive':'cx-sealed-negative')}</button>`}).join('');
}
...
async function executableEconomics(ids){
  ...
  for(let i=0;i<ids.length;i+=75){
    ...
    out.push(...await rest(`sealed_product_executable_ev_cache?select=sealed_uuid,tcg_low_ev,direct_first_net_ev,collectish_live_out_ev,fixed_tcg_low_ev,fixed_collectish_live_out_ev,modeled_child_units,price_coverage_pct,valuation_basis,model_key,model_version,valuation_as_of,refreshed_at&sealed_uuid=in.(${part})`));
  }
  return out;
}
...
function econTable(cards){
  ...
  return `<div class="cx-econ-legend"><span class="retail">RETAIL / ACQUIRE</span><span class="reference">TCGM = MARKET REF</span><span class="exit">EXIT / SELL</span></div><div class="cx-sealed-econ-wrap" data-no-detail-swipe><table class="cx-sealed-econ"><thead><tr><th class="sticky-name">Card</th><th>Qty</th><th>Finish</th><th>TCGL</th><th>L+S</th><th>TCGM</th><th>CKR</th><th>MP</th><th>MKM</th><th>CKBL</th><th>TCGD</th></tr></thead><tbody>${cards.map(c=>{const u=sourceUrls(c),b=bestClasses(c),rc=v=>Number(v)>0&&Math.abs(Number(v)-b.retail)<.005?'cx-econ-best-retail':'',xc=v=>Number(v)>0&&Math.abs(Number(v)-b.exit)<.005?'exit cx-econ-best-exit':'exit';return `<tr><td class="sticky-name">${scoutAnchor(`<strong>${esc(c.card_name)}</strong><small>${esc(c.set_code||'')} #${esc(c.collector_number||'—')}<span class="cx-econ-mobile-meta"> · ×${Number(c.quantity||0).toLocaleString()} · ${esc(human(c.finish||''))}</span></small>`,u.scout)}</td><td>${Number(c.quantity||0).toLocaleString()}</td><td>${esc(human(c.finish||''))}</td>${cell(c.tcg_low,u.tcg,rc(c.tcg_low))}${cell(c.tcg_low_with_shipping,u.tcg,rc(c.tcg_low_with_shipping))}${cell(c.tcg_market,u.tcg)}${cell(c.cardkingdom_retail,u.ck,rc(c.cardkingdom_retail))}${cell(c.manapool_retail,u.mana,rc(c.manapool_retail))}${cell(c.cardmarket_retail,u.mkm,rc(c.cardmarket_retail))}${cell(c.cardkingdom_buylist,u.ckBuy,xc(c.cardkingdom_buylist))}${cell(c.tcg_direct_net,u.direct,xc(c.tcg_direct_net))}</tr>`}).join('')}</tbody><tfoot><tr><th class="sticky-name">Fixed-card totals</th><th></th><th></th><th>${money(t.tcg_low)}</th><th>${money(t.tcg_low_with_shipping)}</th><th>${money(t.tcg_market)}</th><th>${money(t.cardkingdom_retail)}</th><th>${money(t.manapool_retail)}</th><th>${money(t.cardmarket_retail)}</th><th class="exit">${money(t.cardkingdom_buylist)}</th><th class="exit">${money(t.tcg_direct_net)}</th></tr></tfoot></table></div><div class="cx-sealed-econ-note">Prices are per card; fixed-card totals multiply each price by quantity. TCGD uses exact-SKU Direct Low and the modeled Direct fee schedule; it is not a flat percentage haircut.</div>`}

function componentSummary(children,family){if(!children.length)return'';const total=Number(family?.collectish_live_out_ev||0),lowTotal=Number(family?.tcg_low_ev||0),fixed=Number(family?.fixed_collectish_live_out_ev||0),boosterLow=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.tcg_low_ev||0),0),boosterNet=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.collectish_live_out_ev||0),0);return `<section class="cx-sealed-component-summary"><div class="cx-section-title">Included sealed products</div><div class="cx-sealed-child-econ-head"><span>Product</span><span>Qty</span><span>TCG Low / unit</span><span>TCG Low EV</span><span>Collectish EV</span></div>${children.map(c=>{const qty=Number(c.quantity||0),low=Number(c.tcg_low_ev||0),childNet=Number(c.collectish_live_out_ev||0);return `<div class="cx-sealed-component-row"><strong>${esc(c.child_product_name)}</strong><span data-mobile-label="Qty">${qty.toLocaleString()}</span><span>${money(low)}</span><span>${money(qty*low)}</span><span data-mobile-label="Collectish EV">${money(qty*childNet)}</span></div>`}).join('')}<div class="cx-sealed-component-rollup"><span><small>Included packs · TCG Low</small><b>${money(boosterLow)}</b></span><span><small>Fixed-card Collectish EV</small><b>${money(fixed)}</b></span><span class="total"><small>Total Collectish EV</small><b>${money(total)}</b></span><span><small>Included packs · Collectish</small><b>${money(boosterNet)}</b></span><span><small>Total TCG Low EV</small><b>${money(lowTotal)}</b></span></div><div class="cx-sealed-econ-note">Randomized packs use current prices only and exclude SYP/last-known Direct. Fixed cards may be routed separately in Out Optimization.</div></section>`}
...
async function renderDetail(r){
  ...
  try{
    ...
    const totalEv=Number(d.family?.collectish_live_out_ev||0),lowEv=Number(d.family?.tcg_low_ev||0),directEv=Number(d.family?.direct_first_net_ev||0),referenceEv=Number(d.family?.crack_gross_mean_ev||0),composite=d.children.length>0,totalSpread=totalEv&&a!=null?totalEv-a:null,totalRoi=totalSpread!=null&&a>0?100*totalSpread/a:null;
    h.innerHTML=`<h3>${sealedUrl?`<a class="cx-source-anchor" href="${esc(sealedUrl)}" target="_blank" rel="noopener">${esc(r.product_name)}</a>`:esc(r.product_name)}</h3><span class="cx-sub">${esc(r.set_code||'')} · ${esc(type?human(type):human(r.subtype||r.category||''))} · ${esc(r.release_date||'')}</span><div class="cx-sealed-badges">${sc==null?'':`<span class="cx-sealed-badge direct">Scout ${esc(grade(r))} · ${sc.toFixed(1)}/100</span>`}${badges(r)}</div>${languageNote(r)}<div class="cx-sealed-freshness"><span><b>Prices synced</b> ${age(pricesAt)}</span><span><b>Contents synced</b> ${age(d.contents?.source_updated_at)}</span></div><div class="cx-sealed-grid">${stat('Sealed acquisition',money(a),d.price?.captured_at?`TCG observed ${age(d.price.captured_at)}`:'current pipeline value',sealedUrl)}${stat('Gross reference EV',money(referenceEv),'TCG Market · reference only')}${stat('TCG Low EV',money(lowEv),'gross · before fees')}${stat('Direct-first net EV',money(directEv),'live Direct; TCG regular fallback')}${stat('Collectish live-out EV',money(totalEv),composite?`${Number(d.family?.modeled_child_units||0)} sealed packs + fixed cards`:'best current executable route per pull')}${stat('Collectish spread',money(totalSpread),totalRoi==null?'':`${totalRoi>=0?'+':''}${totalRoi.toFixed(1)}%`)}</div>${provenance(d.family,composite)}${r.blocker?`<div class="cx-sealed-summary"><strong>Current blocker:</strong> ${esc(human(r.blocker))}.</div>`:''}${componentSummary(d.children,d.family)}<div class="cx-section-title cx-sealed-econ-title">Component economics</div>${econTable(d.cards)}`;
