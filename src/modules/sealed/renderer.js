...
function collectorForecastSummary(rows=[]){
  ...
  const primaryDays=ready.filter(x=>x.promotion_status==='PRIMARY').map(x=>`${Number(x.horizon_days)}d`),shadowDays=ready.filter(x=>x.promotion_status!=='PRIMARY').map(x=>`${Number(x.horizon_days)}d`),demand=features.units_sold_30d==null?'Demand history unavailable':`${Number(features.units_sold_30d).toLocaleString()} items / 30d`;
  const gate=primaryDays.length&&shadowDays.length?`Similarity is PRIMARY at ${primaryDays.join(' and ')}; ${shadowDays.join(' and ')} remains SHADOW with its pooled cohort primary.`:primaryDays.length?'Similarity cleared the leakage-safe promotion gate at every horizon.':`Pooled cohort remains primary; similarity is shadow because it has not beaten the pooled error benchmark at any horizon.`;
