...
function renderProductRows(visible,selected){
  const types=setMap();
  return visible.map(r=>{const a=acquisition(r),sc=score(r),type=types.get(String(r.set_code||'').toUpperCase()),family=familyEconomics(r),floor=componentFloor(r),ev=modeledEv(r),spread=modeledSpread(r),roi=spread!=null&&a>0?100*spread/a:null,evSub=floor?'known cards only · sample pack excluded':family?.practical_action?human(family.practical_action):'liquidity + labor adjusted';return `<button type="button" class="cx-sealed-row ${String(r.sealed_uuid)===String(selected)?'selected':''}" data-deck="${esc(r.sealed_uuid)}"><div class="cx-sealed-name"><strong>${esc(r.product_name)}</strong><small>${esc(human(r.subtype||r.category||type||''))} · ${esc(r.release_date||'')}</small><div class="cx-sealed-badges">${sc==null?`<span class="cx-sealed-badge risk">${floor?'COMPONENT FLOOR':'MODEL PENDING'}</span>`:`<span class="cx-sealed-badge direct">${esc(grade(r))} · ${sc.toFixed(1)}</span>`}${badges(r)}</div></div>${metric('Sealed buy',money(a),human(r.lifecycle_status||'catalog only'))}${metric(floor?'Practical floor':'Practical EV',money(ev),evSub)}${metric('TCG Low EV',money(family?.tcg_low_ev),'gross pull value','cx-sealed-hide-mobile')}${metric('Practical spread',spread==null?'—':money(spread),floor?'not scored until pack is modeled':roi==null?'':`${roi>=0?'+':''}${roi.toFixed(1)}%`,spread==null?'cx-sealed-pending':spread>=0?'cx-sealed-positive':'cx-sealed-negative')}</button>`}).join('');
}
...
async function renderDetail(r){
  ...
  try{
    ...
    const totalEv=d.family?.practical_liquidation_ev==null?null:Number(d.family.practical_liquidation_ev),optimisticEv=Number(d.family?.collectish_live_out_ev||0),lowEv=Number(d.family?.tcg_low_ev||0),median=d.family?.practical_median_estimate==null?null:Number(d.family.practical_median_estimate),p10=d.family?.practical_p10_estimate==null?null:Number(d.family.practical_p10_estimate),concentration=Number(d.family?.top10_practical_ev_share_pct),composite=d.children.length>0,totalSpread=!detailFloor&&totalEv&&a!=null?totalEv-a:null,totalRoi=totalSpread!=null&&a>0?100*totalSpread/a:null;
    h.innerHTML=`<h3>${sealedUrl?`<a class="cx-source-anchor" href="${esc(sealedUrl)}" target="_blank" rel="noopener">${esc(r.product_name)}</a>`:esc(r.product_name)}</h3><span class="cx-sub">${esc(r.set_code||'')} · ${esc(type?human(type):human(r.subtype||r.category||''))} · ${esc(r.release_date||'')}</span><div class="cx-sealed-badges">${detailFloor?'<span class="cx-sealed-badge risk">COMPONENT FLOOR</span>':sc==null?'':`<span class="cx-sealed-badge direct">Scout ${esc(detailGrade)} · ${sc.toFixed(1)}/100</span>`}${badges(r)}</div>${languageNote(r)}<div class="cx-sealed-freshness"><span><b>Prices synced</b> ${age(pricesAt)}</span><span><b>Contents synced</b> ${age(d.contents?.source_updated_at)}</span></div><div class="cx-sealed-grid">${stat('Sealed acquisition',money(a),d.price?.captured_at?`TCG observed ${age(d.price.captured_at)}`:'current pipeline value',sealedUrl)}${stat(detailFloor?'Known-card practical floor':'Practical liquidation EV',money(totalEv),detailFloor?'sample pack excluded · not scored':d.family?.practical_action?human(d.family.practical_action):'liquidity + labor adjusted')}${stat('Practical median est.',money(median),detailFloor?'requires complete pack model':d.family?.distribution_estimate_basis||'scaled simulation shape')}${stat('Practical P10 est.',money(p10),detailFloor?'requires complete pack model':'downside estimate')}${stat('TCG Low EV',money(lowEv),'gross · Market excluded')}${stat('Optimistic live-out',money(optimisticEv),composite?`${Number(d.family?.modeled_child_units||0)} packs + fixed cards`:'best current route per pull')}${stat('Top-10 concentration',Number.isFinite(concentration)?pct(concentration):'—','share of practical EV')}${stat('Practical spread',money(totalSpread),detailFloor?'not scored until pack is modeled':totalRoi==null?'':`${totalRoi>=0?'+':''}${totalRoi.toFixed(1)}%`)}</div>${r.blocker?`<div class="cx-sealed-summary"><strong>Current blocker:</strong> ${esc(human(r.blocker))}.</div>`:''}${componentSummary(d.children,d.family)}<div class="cx-section-title cx-sealed-econ-title">Component economics</div>${econTable(d.cards)}`;
