...
function lifecycleSummary(x){if(!x)return'';const e=x.lifecycle_evidence||{},move=n=>n==null?'—':`${Number(n)>=0?'+':''}${Number(n).toFixed(1)}%`;return `<section class="cx-sealed-component-summary"><div class="cx-section-title">Collector-box lifecycle</div><div class="cx-sealed-grid">${stat('Pattern',esc(x.lifecycle_state||'MIXED'),`${Number(x.history_days||0)} history days · ${Number(x.observation_count||0)} observations`)}${stat('Trajectory action',esc(x.trajectory_action||'OBSERVE'),`${esc(x.calibration_confidence||'LOW')} calibration · ${Number(x.calibration_samples||0)} tests`)}${stat('30d / 90d move',`${move(e.change_30d_pct)} / ${move(e.change_90d_pct)}`,'TCG Market trajectory only')}${stat('Sales demand',`${Number(e.units_30d||0).toLocaleString()} / 30d`,`${Number(e.units_90d||0).toLocaleString()} units / 90d`)}${stat('Current supply',e.total_listings==null?'—':Number(e.total_listings).toLocaleString(),'TCGplayer listings')}${stat('Range position',`${move(e.above_history_low_pct)} above low`,`${move(e.below_history_high_pct)} below high`)}</div><div class="cx-sealed-econ-note">${esc(x.action_caveat||'Observed pattern only.')} TCG Market is never used as liquidation EV.</div></section>`}
...
async function loadDetailData(r){
  ...
  return loadResource(`sealed.detail:v6:${r.sealed_uuid}`,async()=>{
    const [cards,price,contents,children,analogs,backtests,lifecycles]=await Promise.all([
    ...
    return{cards:cards||[],price:(price||[])[0]||null,contents:(contents||[])[0]||null,family:byId.get(String(r.sealed_uuid))||null,children:(children||[]).map(c=>({...c,...byId.get(String(c.child_sealed_uuid))})),analogs:analogs||[],backtest:(backtests||[])[0]||null,lifecycle:(lifecycles||[])[0]||null};
  },{ttl:5*60*1000});
}
