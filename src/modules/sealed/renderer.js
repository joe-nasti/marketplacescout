...
async function renderDetail(r){
  ...
  try{
    ...
    const totalEv=Number(d.family?.collectish_live_out_ev||0),lowEv=Number(d.family?.tcg_low_ev||0),directEv=Number(d.family?.direct_first_net_ev||0),composite=d.children.length>0,totalSpread=totalEv&&a!=null?totalEv-a:null,totalRoi=totalSpread!=null&&a>0?100*totalSpread/a:null;
    h.innerHTML=`<h3>${sealedUrl?`<a class="cx-source-anchor" href="${esc(sealedUrl)}" target="_blank" rel="noopener">${esc(r.product_name)}</a>`:esc(r.product_name)}</h3><span class="cx-sub">${esc(r.set_code||'')} · ${esc(type?human(type):human(r.subtype||r.category||''))} · ${esc(r.release_date||'')}</span><div class="cx-sealed-badges">${sc==null?'':`<span class="cx-sealed-badge direct">Scout ${esc(grade(r))} · ${sc.toFixed(1)}/100</span>`}${badges(r)}</div>${languageNote(r)}<div class="cx-sealed-freshness"><span><b>Prices synced</b> ${age(pricesAt)}</span><span><b>Contents synced</b> ${age(d.contents?.source_updated_at)}</span></div><div class="cx-sealed-grid">${stat('Sealed acquisition',money(a),d.price?.captured_at?`TCG observed ${age(d.price.captured_at)}`:'current pipeline value',sealedUrl)}${stat('Collectish live-out EV',money(totalEv),composite?`${Number(d.family?.modeled_child_units||0)} sealed packs + fixed cards`:'best current executable route per pull')}${stat('TCG Low EV',money(lowEv),'gross · Market excluded')}${stat('Direct-first net EV',money(directEv),'live Direct; TCG regular fallback')}${stat('Collectish spread',money(totalSpread),totalRoi==null?'':`${totalRoi>=0?'+':''}${totalRoi.toFixed(1)}%`)}</div>${r.blocker?`<div class="cx-sealed-summary"><strong>Current blocker:</strong> ${esc(human(r.blocker))}.</div>`:''}${componentSummary(d.children,d.family)}<div class="cx-section-title cx-sealed-econ-title">Component economics</div>${econTable(d.cards)}`;
