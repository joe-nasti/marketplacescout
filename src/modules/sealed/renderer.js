...
async function renderDetail(r){
  ...
  try{
    ...
    h.innerHTML=`<h3>${sealedUrl?`<a class="cx-source-anchor" href="${esc(sealedUrl)}" target="_blank" rel="noopener">${esc(r.product_name)}</a>`:esc(r.product_name)}</h3><span class="cx-sub">${esc(r.set_code||'')} · ${esc(type?human(type):human(r.subtype||r.category||''))} · ${esc(r.release_date||'')}</span><div class="cx-sealed-badges">${detailFloor?'<span class="cx-sealed-badge risk">COMPONENT FLOOR</span>':sc==null?'':`<span class="cx-sealed-badge direct">Scout ${esc(detailGrade)} · ${sc.toFixed(1)}/100</span>`}${badges(r)}</div>${languageNote(r)}<div class="cx-sealed-freshness"><span><b>Prices synced</b> ${age(pricesAt)}</span><span><b>Contents synced</b> ${age(d.contents?.source_updated_at)}</span></div><div class="cx-section-title">EV sensitivity</div><div class="cx-sealed-grid">${stat('Cash floor',money(cashFloor),'Card Kingdom cash buylist; missing cards $0')}${stat(detailFloor?'Known-card practical floor':'Practical EV',money(totalEv),detailFloor?'sample pack excluded · not scored':'fees + liquidity + labor adjusted')}${stat('Optimistic EV',money(optimisticEv),composite?`${Number(d.family?.modeled_child_units||0)} packs + fixed cards`:'best current route after marketplace fees')}${stat('Sealed acquisition',money(a),d.price?.captured_at?`TCG observed ${age(d.price.captured_at)}`:'current pipeline value',sealedUrl)}${stat('Practical median est.',money(median),detailFloor?'requires complete pack model':d.family?.distribution_estimate_basis||'scaled simulation shape')}${stat('Practical P10 est.',money(p10),detailFloor?'requires complete pack model':'downside estimate')}${stat('TCG Low EV',money(lowEv),'gross reference · Market excluded')}${stat('Top-10 concentration',Number.isFinite(concentration)?pct(concentration):'—','share of practical EV')}${stat('Practical spread',money(totalSpread),detailFloor?'not scored until pack is modeled':totalRoi==null?'':`${totalRoi>=0?'+':''}${totalRoi.toFixed(1)}%`)}</div>${r.blocker?`<div class="cx-sealed-summary"><strong>Current blocker:</strong> ${esc(human(r.blocker))}.</div>`:''}${evAuditSummary(d.family)}${componentSummary(d.children,d.family)}<div class="cx-section-title cx-sealed-econ-title">Component economics</div>${econTable(d.cards)}`;
