...
function scoreOpportunity(row){
  ...
  const spread=(tcg/landed-1)*100,qty=Number(zero.quantity||0),pressure=Number(trend.pressure_score||0);
  const historyReady=trend.signal&&trend.signal!=='building_history',depth=Math.min(25,qty)/25;
  const spreadScore=Math.max(-20,Math.min(60,spread)),trendBoost=historyReady?Math.max(-20,Math.min(20,pressure/5)):0;
...
async function load(){
  ...
  const ids=(rows||[]).map(r=>r.sealed_uuid).filter(Boolean),tcg=[];
...
function ensurePanel(){
  ...
  let panel=root.querySelector('[data-ct0-opportunities]');if(panel)return panel;
  panel=document.createElement('section');panel.dataset.ct0Opportunities='';panel.className='cx-card';
  const layout=root.querySelector('.cx-sealed-layout');if(layout)root.insertBefore(panel,layout);else root.appendChild(panel);
  return panel;
}

async function render(){
  const panel=ensurePanel();if(!panel)return;
  const rows=await load(),ready=rows.filter(r=>r.historyReady),building=rows.filter(r=>!r.historyReady),top=(ready.length?ready:building).slice(0,12);
