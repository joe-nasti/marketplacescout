...
function childOutContribution(children=[]){
  ...
  for(const child of children){
    ...
    else if(basis){randomizedChildren+=1;additiveNet+=value;const channel=basis.includes('sealed_resale')?'Sealed child resale':'Randomized practical out';routes.set(channel,(routes.get(channel)||0)+value)}
...
function renderPanel(row,rows,children=[]){
  ...
  const childRows=children.map(c=>`<div class="cx-out-opt-child"><strong>${Number(c.quantity||0).toLocaleString()} × ${esc(c.child_product_name)}</strong><span>TCG Low ${money(Number(c.quantity||0)*Number(c.tcg_low_ev||0))}</span><span>${String(c.valuation_basis||'').includes('sealed_resale')?'Sealed resale net':'Practical'} ${money(Number(c.quantity||0)*Number(c.practical_liquidation_ev||0))}</span></div>`).join('');
