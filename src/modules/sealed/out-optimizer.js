...
function exportCsv(row,rows,children=[]){
  ...
  const childNet=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.crack_net_mean_ev||0),0),live=Number(row?.optimized_live_out_ev||0)+childNet,potential=Number(row?.optimized_with_syp_potential_ev||0)+childNet;
  const childMeta=children.flatMap(c=>[`Included sealed product,${csvEscape(c.child_product_name)}`,`Quantity,${csvEscape(c.quantity)}`,`Gross EV per unit,${csvEscape(c.crack_gross_mean_ev)}`,`Net EV contribution,${csvEscape(Number(c.quantity||0)*Number(c.crack_net_mean_ev||0))}`]);
...
function renderPanel(row,rows,children=[]){
  const childNet=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.crack_net_mean_ev||0),0),childGross=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.crack_gross_mean_ev||0),0),live=(num(row.optimized_live_out_ev)||0)+childNet,potential=(num(row.optimized_with_syp_potential_ev)||0)+childNet,liveMix=channelMix(rows,'live_best_channel','live_best_component_ev',childNet?[{channel:'Modeled booster out',ev:childNet}]:[]),top=liveMix[0],acq=num(row.sealed_acquisition_price),cash=num(row.cash_floor_ev??row.cardkingdom_buylist_ev);
