...
function exportCsv(row,rows,children=[]){
  ...
  const childNet=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.collectish_live_out_ev||0),0),live=Number(row?.optimized_live_out_ev||0)+childNet,potential=Number(row?.optimized_with_syp_potential_ev||0)+childNet;
  const childMeta=children.flatMap(c=>[`Included sealed product,${csvEscape(c.child_product_name)}`,`Quantity,${csvEscape(c.quantity)}`,`TCG Low EV per unit,${csvEscape(c.tcg_low_ev)}`,`Collectish live-out contribution,${csvEscape(Number(c.quantity||0)*Number(c.collectish_live_out_ev||0))}`]);
...
function renderPanel(row,rows,children=[]){
  const childNet=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.collectish_live_out_ev||0),0),childGross=children.reduce((n,c)=>n+Number(c.quantity||0)*Number(c.tcg_low_ev||0),0),live=(num(row.optimized_live_out_ev)||0)+childNet,potential=(num(row.optimized_with_syp_potential_ev)||0)+childNet,liveMix=channelMix(rows,'live_best_channel','live_best_component_ev',childNet?[{channel:'Randomized live out',ev:childNet}]:[]),top=liveMix[0],acq=num(row.sealed_acquisition_price),cash=num(row.cash_floor_ev??row.cardkingdom_buylist_ev);
  ...
  const childRows=children.map(c=>`<div class="cx-out-opt-child"><strong>${Number(c.quantity||0).toLocaleString()} × ${esc(c.child_product_name)}</strong><span>TCG Low ${money(Number(c.quantity||0)*Number(c.tcg_low_ev||0))}</span><span>Collectish ${money(Number(c.quantity||0)*Number(c.collectish_live_out_ev||0))}</span></div>`).join('');
  return `<section class="cx-out-opt" data-no-detail-swipe><div class="cx-out-opt-head"><div><h4>Out Optimization</h4><p>Outlet-specific fixed-card routing plus current-only live-out value from randomized products.</p></div><div class="cx-out-opt-actions"><button type="button" class="cx-out-opt-btn" data-out-export>Export CSV</button></div></div><div class="cx-out-opt-grid">${stat('Optimized Live Out',money(live),liveRoi==null?'best current routes':`${liveRoi>=0?'+':''}${liveRoi.toFixed(1)}% vs sealed buy`,'cx-out-opt-dream')}${stat('Optimized + SYP',money(potential),potentialRoi==null?'fixed contents only':'fixed contents only · capped SYP','cx-out-opt-potential')}${stat('Included Products Net',money(childNet),children.length?`${money(childGross)} TCG Low EV`:'no sealed children')}${stat('TCG Regular Net',money(row.tcg_regular_net_ev),'fixed cards only')}${stat('ManaPool Net Est',money(row.manapool_net_est_ev),'fixed cards only · slower liquidity')}${stat('CK Cash',money(cash),'fixed cards only')}${stat('SYP Potential',money(row.syp_potential_capped_ev),'fixed cards only')}</div>${childRows?`<div class="cx-out-opt-children">${childRows}</div>`:''}<div class="cx-out-opt-mix">${mix}</div>${warning}<div class="cx-out-opt-note">Randomized products use current executable prices only: live Direct with regular TCG fallback, then the best current Collectish outlet. SYP and last-known Direct are excluded from randomized EV and apply only to known fixed contents.</div><details class="cx-out-opt-details"><summary>View fixed-card routing (${rows.length} lines)</summary><div class="cx-out-opt-table-wrap"><table class="cx-out-opt-table"><thead><tr><th>Card</th><th>Qty</th><th>Live out</th><th>Unit net</th><th>Live EV</th><th>+ SYP route</th><th>Potential EV</th><th>Direct live</th><th>TCG reg</th><th>ManaPool</th><th>CK cash</th><th>SYP potential</th><th>Velocity</th><th>Est. sell</th></tr></thead><tbody>${body}</tbody></table></div></details></section>`;
}
