...
function renderPanel(row,rows,children=[]){
  ...
  const childRows=children.map(c=>`<div class="cx-out-opt-child"><strong>${Number(c.quantity||0).toLocaleString()} × ${esc(c.child_product_name)}</strong><span>TCG Low ${money(Number(c.quantity||0)*Number(c.tcg_low_ev||0))}</span><span>Practical ${money(Number(c.quantity||0)*Number(c.practical_liquidation_ev||0))}</span></div>`).join('');
