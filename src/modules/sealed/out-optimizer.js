...
function injectStyles(){
  ...
  style.textContent=`
    ...
    @media(max-width:760px){.cx-out-opt-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.cx-out-opt-head{display:block}.cx-out-opt-actions{margin-top:8px}}
  `;
  document.head.appendChild(style);
}

function csvEscape(v){const s=String(v??'');return /[",\n]/.test(s)?`"${s.replaceAll('"','""')}"`:s}
function exportCsv(row,rows){
  const cols=[
  ...
  const meta=[`Product,${csvEscape(row?.product_name||'')}`,`Sealed acquisition,${csvEscape(row?.sealed_acquisition_price)}`,`Optimized Live Out EV,${csvEscape(row?.optimized_live_out_ev)}`,`Optimized + SYP Potential EV,${csvEscape(row?.optimized_with_syp_potential_ev)}`,`TCG Regular Net EV,${csvEscape(row?.tcg_regular_net_ev)}`,`ManaPool Net Est EV,${csvEscape(row?.manapool_net_est_ev)}`,`CK Cash EV,${csvEscape(row?.cash_floor_ev??row?.cardkingdom_buylist_ev)}`,''];
...
function channelMix(rows,key='live_best_channel',evKey='live_best_component_ev'){
...
function renderPanel(row,rows){
  const liveMix=channelMix(rows),top=liveMix[0],live=num(row.optimized_live_out_ev),potential=num(row.optimized_with_syp_potential_ev),acq=num(row.sealed_acquisition_price),cash=num(row.cash_floor_ev??row.cardkingdom_buylist_ev);
  ...
  return `<section class="cx-out-opt" data-no-detail-swipe><div class="cx-out-opt-head"><div><h4>Out Optimization</h4><p>Outlet-specific net EV plus a transparent dream routing plan. Market prices are reference only; the optimizer routes card-by-card to the highest modeled out.</p></div><div class="cx-out-opt-actions"><button type="button" class="cx-out-opt-btn" data-out-export>Export CSV</button></div></div><div class="cx-out-opt-grid">${stat('Optimized Live Out',money(live),liveRoi==null?'best current routes':`${liveRoi>=0?'+':''}${liveRoi.toFixed(1)}% vs sealed buy`,'cx-out-opt-dream')}${stat('Optimized + SYP',money(potential),potentialRoi==null?'hypothetical capped Direct':'hypothetical · capped SYP','cx-out-opt-potential')}${stat('TCG Regular Net',money(row.tcg_regular_net_ev),'single-out scenario')}${stat('ManaPool Net Est',money(row.manapool_net_est_ev),'92.1% merchandise · slower liquidity')}${stat('CK Cash',money(cash),'current cash buylist')}${stat('SYP Potential',money(row.syp_potential_capped_ev),'capped hypothetical lane')}</div><div class="cx-out-opt-mix">${mix}</div>${warning}<div class="cx-out-opt-note">ManaPool is intentionally not discounted inside the literal dream optimizer; its concentration is surfaced instead. SYP potential is capped at min(last Direct Low, 130% of current Market/listing) × 80% and respects known SYP quantity. TCG sell-through days use exact-SKU observed velocity when available.</div><details class="cx-out-opt-details"><summary>View card-by-card routing (${rows.length} lines)</summary><div class="cx-out-opt-table-wrap"><table class="cx-out-opt-table"><thead><tr><th>Card</th><th>Qty</th><th>Live out</th><th>Unit net</th><th>Live EV</th><th>+ SYP route</th><th>Potential EV</th><th>Direct live</th><th>TCG reg</th><th>ManaPool</th><th>CK cash</th><th>SYP potential</th><th>Velocity</th><th>Est. sell</th></tr></thead><tbody>${body}</tbody></table></div></details></section>`;
}

async function attachOptimizer(event){
  const seq=++renderSeq,row=event?.detail?.row,id=event?.detail?.id;if(!row||!id)return;
  ...
  try{
    ...
    if(seq!==renderSeq||!rows?.length)return;
    target.insertAdjacentHTML('beforebegin',renderPanel(row,rows));
    detail.querySelector('[data-out-export]')?.addEventListener('click',()=>exportCsv(row,rows));
