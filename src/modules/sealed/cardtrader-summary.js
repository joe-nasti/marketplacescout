...
async function load(force=false){
  ...
  try{
    ...
    const ct=rows||[],opps=ct.filter(x=>x.raw_json?.ct_zero_opportunity?.eligible===true),strong=opps.filter(x=>x.raw_json?.ct_zero_opportunity?.tier==='strong'),tight=opps.filter(x=>['tightening','strong_tightening'].includes(x.raw_json?.ct_zero_trend?.signal)),stocked=ct.filter(x=>Number(x.raw_json?.ct_zero?.quantity||0)>0),last=ct.map(x=>x.captured_at).filter(Boolean).sort().at(-1)||null;
    bar.innerHTML=`<span class="cx-ct0-pill-copy"><strong>CT0</strong><span>${opps.length} opps · ${strong.length} strong${tight.length?` · ${tight.length} tightening`:''} · ${stocked.length} stocked</span>${last?`<small>${new Date(last).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}</small>`:''}</span><button type="button" class="cx-ct0-pill-action" data-ct0-summary-action="opps">View</button><button type="button" class="cx-ct0-pill-dismiss" data-ct0-dismiss aria-label="Dismiss CardTrader Zero summary">×</button>`;
    lastAt=Date.now();
