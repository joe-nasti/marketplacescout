...
async function renderCardTrader(event){
  ...
  const bpRows=row.product_id?await rest(`cardtrader_blueprints?select=blueprint_id,raw_json&blueprint_id=eq.${encodeURIComponent(row.product_id)}&limit=1`).catch(()=>[]):[];
  if(current!==seq)return;
  const bp=(bpRows||[])[0]||{},webUrl=bp.raw_json?.web_url||null;
