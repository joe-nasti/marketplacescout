...
(() => {
  ...
  function transportControl(){
    ...
    const endpoint=window.CollectishAskStreaming?.endpoint||String(cfg.askStreamUrl||(cfg.supabaseUrl?`${String(cfg.supabaseUrl).replace(/\/$/,'')}/functions/v1/ask-collectish-stream`:''));
    return `<div class="cx-v3-diag-line cx-ask-transport"><b>Streaming transport:</b> <strong>${endpoint?'ACTIVE':'INACTIVE'}</strong><span>${endpoint?` · Supabase fast path · ${esc(endpoint)}`:' · fast stream unavailable; Ask will use the V3 tool path only'}</span></div>`;
  }
  function prefetchControl(){
    const on=prefetchEnabled();
    return `<div class="cx-v3-diag-line cx-ask-prefetch-control"><b>AI Speculative Prefetch</b> <button type="button" data-ask-prefetch-toggle aria-pressed="${on?'true':'false'}">${on?'ON':'OFF'}</button><span>Opt-in; fast Scout questions use Supabase streaming.</span></div>`;
  }
  function latencyControl(){
    const s=window.CollectishAskLatency?.stats?.()||{},last=s.last||{},avg=s.avg||{};
    const ms=v=>v==null?'—':`${Number(v).toLocaleString()} ms`;
    return `<div class="cx-v3-diag-line cx-ask-latency"><b>Streaming latency</b> <span>Last TTFT ${ms(last.ttftMs)} · Avg TTFT ${ms(avg.ttftMs)} · Headers ${ms(avg.headersMs)} · Total ${ms(avg.totalMs)} · ${Number(s.count||0)} samples${last.cached?' · last cached':''}${last.prefetched?' · prefetch cache warm':''}</span> <button type="button" data-ask-latency-clear>Clear samples</button></div>`;
  }
  function bindControls(card){
    const b=card?.querySelector('[data-ask-prefetch-toggle]');
    if(b)b.onclick=()=>{
    ...
    const clear=card?.querySelector('[data-ask-latency-clear]');
    if(clear)clear.onclick=()=>{window.CollectishAskLatency?.clear?.();render()};
  }
  async function render(){
    ...
    if(!card){const grid=host.querySelector('.cx-grid')||host;card=document.createElement('div');card.className='cx-card cx-span-12 cx-v3-safe-diagnostics';card.innerHTML='<div class="cx-section-title">Ask Collectish V3 · operations diagnostics</div>'+transportControl()+prefetchControl()+latencyControl()+'<div class="cx-empty">Loading…</div>';grid.append(card);bindControls(card)}
    loading=true;
    try{
