...
(() => {
  ...
  function transportControl(){
    const url=String(window.COLLECTISH_CONFIG?.askStreamUrl||'').trim();
    const sameOriginEligible=location.protocol==='https:'&&!location.hostname.endsWith('github.io');
    const endpoint=url||(sameOriginEligible?new URL('/api/ask-collectish',location.origin).href:'');
    return `<div class="cx-v3-diag-line cx-ask-transport"><b>Streaming transport:</b> <strong>${endpoint?'ACTIVE':'INACTIVE'}</strong><span>${endpoint?` · ${esc(endpoint)}`:' · endpoint not configured; Ask is using the legacy Supabase JSON path'}</span></div>`;
  }
