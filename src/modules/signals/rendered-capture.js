...
async function save(){
  ...
  try{
    ...
    result=null;renderResult();document.getElementById('cxRenderedText').value='';if(msg)msg.textContent=`Saved ${selected.length} rendered-text signal${selected.length===1?'':'s'}.`;document.dispatchEvent(new CustomEvent('collectish:intel-changed',{detail:{source:'rendered-capture'}}));
