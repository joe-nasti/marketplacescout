...
let mode='scan',filter='all',query='',salesResponse=new Map(),salesLoadedAt=0,salesLoading=null;
...
function salesFor(card){return salesResponse.get(String(card||'').trim().toLowerCase())||null}
async function loadSalesResponse(force=false){
  if(salesLoading)return salesLoading;
  if(!force&&salesLoadedAt&&Date.now()-salesLoadedAt<SALES_TTL)return salesResponse;
  salesLoading=rest('marketplace_signal_card_sales_response?select=card_name,evidence_status,evidence_level,time_to_3_transactions_days,post_signal_transactions_to_date,transaction_velocity_lift_30d_pct,coverage_status,coverage_pct&limit=500')
    .then(rows=>{salesResponse=new Map((rows||[]).map(r=>[String(r.card_name||'').trim().toLowerCase(),r]));salesLoadedAt=Date.now();return salesResponse})
    .catch(()=>salesResponse)
    .finally(()=>{salesLoading=null;renderShell()});
  return salesLoading;
}
...
function buildRows(){
  ...
  for(const r of actionableRows()){
    ...
    row.sales_response=salesFor(row.card_name);map.set(row.key,row);
  }
  for(const item of intelItems()){
    ...
    for(const e of entities){
      ...
      const cardName=e.entity_name||'Unknown card';
      map.set(key,{key,sku_id:null,product_id:e.product_id||null,scryfall_id:e.scryfall_id||null,card_name:cardName,set_name:e.set_code||'',printing:'',kind:'watch',signal:item.signal_stage==='leading'?'Leading':item.signal_stage==='confirming'?'Confirming':'Watch',primary_signal:item.summary||item.title||'External market intelligence',evidence:1,confidence:Math.round(Math.max(0,Math.min(1,Number(item.confidence??e.confidence??0.5)))*100),scout:null,roi:null,buy:null,direct_net:null,liquidity:'',updated_at:item.observed_at||item.published_at||item.created_at||null,direction:item.direction||'neutral',stage:item.signal_stage||'unclassified',intel_count:1,sales_response:salesFor(cardName)});
    }
  }
...
function salesConfirmation(r){
  const s=r.sales_response;if(!s)return'';const tx=Number(s.post_signal_transactions_to_date||0),days=Number(s.time_to_3_transactions_days||0),lift=Number(s.transaction_velocity_lift_30d_pct),coverage=Number(s.coverage_pct||0);
  if(s.evidence_status==='confirmed')return `<span class="cx-sv-sales-confirm">${uiEvidenceMarker('verified',`Card-level NM English marketplace sales confirmed: ${tx} post-signal transactions${days?` with 3 reached in ${days}d`:''}. Direct vs non-Direct is not identified.`)}<b>${Math.min(99,tx)}tx${days?`/${days}d`:''}</b>${Number.isFinite(lift)&&lift!==0?`<i>${lift>0?'+':''}${Math.round(lift)}%</i>`:''}</span>`;
  if(tx>=2)return `<span class="cx-sv-sales-confirm">${uiEvidenceMarker('inferred',`${tx} card-level NM English marketplace transactions observed after the signal; 3 are required for normal confirmation.`)}<b>${tx}tx</b></span>`;
  if(tx===1)return `<span class="cx-sv-sales-confirm">${uiEvidenceMarker('caution','One card-level NM English marketplace transaction observed after the signal; evidence is anecdotal.') }<b>1tx</b></span>`;
  if(s.evidence_status==='partial_coverage'&&coverage>0)return `<span class="cx-sv-sales-confirm">${uiEvidenceMarker('unmeasured',`Sales coverage is partial (${Math.round(coverage)}% of mapped NM English SKUs).`)}</span>`;
  return'';
}
function rowHtml(r){const edge=r.roi==null?'—':pct(r.roi),price=r.buy==null?'—':money(r.buy),evidenceText=r.evidence?String(r.evidence):'1',scout=r.scout==null?'—':Math.round(r.scout),confMarker=uiEvidenceMarker('inferred','Confidence is modeled/inferred, not a directly observed market fact.');return `<button type="button" class="cx-sv-row" data-sv-open data-sku="${esc(r.sku_id||'')}" data-product="${esc(r.product_id||'')}" data-scryfall="${esc(r.scryfall_id||'')}" data-card="${esc(r.card_name)}"><span class="cx-sv-card"><strong>${esc(r.card_name)}</strong><small>${esc([r.set_name,r.printing].filter(Boolean).join(' · '))}</small></span><span class="cx-sv-signal">${stageChip(r)}${salesConfirmation(r)}<small>${esc(r.primary_signal)}</small></span><span class="cx-sv-num"><strong>${esc(evidenceText)}</strong><small>evd</small></span><span class="cx-sv-num"><strong>${esc(String(r.confidence))}${confMarker}</strong><small>conf</small></span><span class="cx-sv-num"><strong>${esc(String(scout))}</strong><small>Scout</small></span><span class="cx-sv-price"><strong>${esc(price)}</strong><small>${esc(edge)} edge</small></span><span class="cx-sv-move ${r.direction}"><strong>${esc(direction(r))}</strong><small>${esc(age(r.updated_at))}</small></span></button>`}
...
document.addEventListener('collectish:lazy-page-loaded',e=>{if(e.detail?.page==='signals')setTimeout(()=>{renderShell();void loadSalesResponse()},100)});document.addEventListener('collectish:page-change',e=>{if(e.detail?.page==='signals')setTimeout(()=>{renderShell();void loadSalesResponse()},80)});document.addEventListener('collectish:intel-changed',schedule);document.addEventListener('collectish:actionable-emerging-changed',schedule);document.addEventListener('collectish:competitive-changed',schedule);document.addEventListener('collectish:commander-intel-changed',schedule);queueMicrotask(()=>{renderShell();void loadSalesResponse()});
