...
function filteredRows(){
  ...
  return buildRows().filter(r=>(filter==='all'||r.kind===filter)&&(!q||`${r.card_name} ${r.set_name} ${r.primary_signal}`.toLowerCase().includes(q)));
}
...
function stageChip(r){const cls=r.kind==='action'?'cx-sv-action':r.kind==='emerging'?'cx-sv-emerging':'cx-sv-watch';return `<span class="cx-sv-chip ${cls}">${esc(r.signal)}</span>`}
...
function rowHtml(r){
  ...
  return `<button type="button" class="cx-sv-row" data-sv-open data-sku="${esc(r.sku_id||'')}" data-product="${esc(r.product_id||'')}" data-scryfall="${esc(r.scryfall_id||'')}" data-card="${esc(r.card_name)}">
    ...
    <span class="cx-sv-move ${r.direction}"><strong>${esc(direction(r))}</strong><small>${esc(age(r.updated_at))}</small></span>
  </button>`;
}
function renderScan(){
  const rows=filteredRows(),all=buildRows();
  const action=all.filter(r=>r.kind==='action').length,emerging=all.filter(r=>r.kind==='emerging').length,watch=all.filter(r=>r.kind==='watch').length;
  const sources=new Set(intelItems().map(x=>x.source_url).filter(Boolean)).size;
...
function renderShell(){
  ...
  let shell=document.getElementById('cxSignalsVnext');
  if(!shell){shell=document.createElement('section');shell.id='cxSignalsVnext';shell.className='cx-signals-vnext';const head=h.querySelector('.cx-page-head');if(head)head.insertAdjacentElement('afterend',shell);else h.prepend(shell)}
  shell.innerHTML=`<div class="cx-sv-nav"><button type="button" data-sv-mode="scan" class="${mode==='scan'?'active':''}">Scan</button><button type="button" data-sv-mode="feed" class="${mode==='feed'?'active':''}">Feed</button><button type="button" data-sv-mode="sources" class="${mode==='sources'?'active':''}">Sources</button><span>Dense market intelligence</span></div><div id="cxSvBody">${mode==='scan'?renderScan():''}</div>`;
  applyMode();
}
function legacyPanels(){
  const h=host();if(!h)return[];
  return [...h.children].filter(el=>el.id!=='cxSignalsVnext'&&!el.classList.contains('cx-page-head'));
}
function applyMode(){
  const h=host();if(!h)return;
  const scan=mode==='scan';
  h.classList.toggle('cx-sv-scan-mode',scan);
  for(const el of legacyPanels())el.hidden=scan;
  if(mode==='feed'){
    for(const el of legacyPanels())el.hidden=false;
    document.getElementById('cxSourceCollectors')?.removeAttribute('open');
  }
  if(mode==='sources'){
    for(const el of legacyPanels())el.hidden=false;
    const collectors=document.getElementById('cxSourceCollectors');if(collectors){collectors.open=true;setTimeout(()=>collectors.scrollIntoView({block:'start',behavior:'smooth'}),40)}
    const feed=document.getElementById('cxSignalsFeed');if(feed)feed.closest('section')?.classList.add('cx-sv-source-muted');
  }else document.querySelector('.cx-sv-source-muted')?.classList.remove('cx-sv-source-muted');
}
...
document.addEventListener('click',e=>{
  const m=e.target.closest?.('[data-sv-mode]');if(m){e.preventDefault();setMode(m.dataset.svMode);return}
  const f=e.target.closest?.('[data-sv-filter]');if(f){e.preventDefault();filter=f.dataset.svFilter;renderShell();return}
  const row=e.target.closest?.('[data-sv-open]');if(row){e.preventDefault();openScout(row);return}
  const more=e.target.closest?.('[data-sv-more]');if(more){e.preventDefault();filter='all';setMode('feed')}
},true);
...
document.addEventListener('collectish:lazy-page-loaded',e=>{if(e.detail?.page==='signals')setTimeout(renderShell,100)});
document.addEventListener('collectish:page-change',e=>{if(e.detail?.page==='signals')setTimeout(renderShell,80)});
document.addEventListener('collectish:intel-changed',schedule);
document.addEventListener('collectish:actionable-emerging-changed',schedule);
document.addEventListener('collectish:competitive-changed',schedule);
document.addEventListener('collectish:commander-intel-changed',schedule);
