...
export function installSignalsDiscovery(){if(installed)return;installed=true;document.addEventListener('click',routeClick,true);document.addEventListener('input',routeInput,true);document.addEventListener('collectish:page-change',e=>{if(e.detail?.page==='signals')queueMicrotask(ensureSurface);else leaveDiscovery()});document.addEventListener('collectish:lazy-page-loaded',onSignalsLifecycle);document.addEventListener('collectish:ready',()=>queueMicrotask(ensureSurface));document.addEventListener('collectish:intel-changed',refreshIfActive);queueMicrotask(ensureSurface)}
