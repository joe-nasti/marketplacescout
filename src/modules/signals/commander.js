...
async function load({force=false}={}){
  ...
  loading=Promise.allSettled([
    ...
    document.dispatchEvent(new CustomEvent('collectish:commander-intel-changed',{detail:{edh:edhRows.length,cedh:cedhRows.length,edhError,cedhError}}));
    return edhRows;
...
async function syncCedh(){
  ...
  if(btn){btn.disabled=true;btn.textContent='Refreshing…'}syncMessage='Importing recent cEDH tournaments from TopDeck.gg…';render();
  try{
    ...
    syncMessage=`Imported ${data.events||0} new cEDH event${Number(data.events)===1?'':'s'}, ${data.decks||0} tournament entries and ${data.cards||0} structured card rows from TopDeck.gg.${data.skipped_imported?` Skipped ${data.skipped_imported} already-imported event${Number(data.skipped_imported)===1?'':'s'}.`:''}${data.errors?` ${data.errors} event${Number(data.errors)===1?'':'s'} failed.`:''}`;
