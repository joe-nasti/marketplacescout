...
function responseForThesis(thesis){
  ...
  return rows.sort((a,b)=>Number(b.unpriced_catalyst_gap_score||0)-Number(a.unpriced_catalyst_gap_score||0)||Number(b.catalyst_impact_score||0)-Number(a.catalyst_impact_score||0)||Number(b.convergence_score||0)-Number(a.convergence_score||0))[0];
}
...
function expectationEvidence(r){if(!r)return'';const mode=String(r.expected_reaction_confidence||'prior_only'),n=Number(r.cohort_mature_signals||0);if(mode==='prior_only')return 'Expected reaction is still prior-driven; cohort history is not mature enough yet.';if(mode==='source_calibrated')return `Expected reaction uses source-level history; exact creator/event/price cohort is still sparse.`;return `Expected reaction is calibrated with ${n} mature creator × event × price-band outcome${n===1?'':'s'}.`}
function scoreEvidence(r){if(!r)return'';return `Conviction ${Number(r.content_conviction_score??r.creator_conviction_score??0)} · Catalyst ${Number(r.catalyst_impact_score||0)} · Expected ${Number(r.expected_market_reaction_score||0)} · Gap ${Number(r.unpriced_catalyst_gap_score||0)} · Convergence ${Number(r.convergence_score||0)} · Market ${Number(r.market_response_score||0)}`}
function decorateSignals(){
  ...
  for(const thesis of thesisData()){
    ...
    const market=response?`<span class="cx-signal-stage"><strong>Gap ${Number(response.unpriced_catalyst_gap_score||0)}</strong> · Expected ${Number(response.expected_market_reaction_score||0)} · Market ${Number(response.market_response_score||0)} · Catalyst ${Number(response.catalyst_impact_score||0)} · Conviction ${Number(response.content_conviction_score??response.creator_conviction_score??0)} · Convergence ${Number(response.convergence_score||0)}</span><small class="cx-sub">${esc(pretty(response.unpriced_catalyst_gap_state||'small_gap'))} · ${esc(expectationEvidence(response))} · ${esc(scopeEvidence(response))} · ${esc(responseEvidence(response))}</small>`:'';
...
function decorateScoutDetail(sku){
  ...
  const expected=Number(topResponse?.expected_market_reaction_score??0),gap=Number(topResponse?.unpriced_catalyst_gap_score??0);
  ...
  const gapStat=topResponse?`<div class="cx-v5-stat"><span>Expected reaction / unpriced gap</span><strong>${expected} / ${gap}</strong><small>${esc(pretty(topResponse.unpriced_catalyst_gap_state||'small_gap'))} · ${esc(expectationEvidence(topResponse))}</small></div>`:'';
  const rows=latest.map(t=>{const r=responseForThesis(t);return `<div class="cx-v5-stat"><span>${esc(pretty(t.primary_event?.event_type))}</span><strong>${esc(t.channel_name||'Creator')}${r?` · gap ${Number(r.unpriced_catalyst_gap_score||0)}`:''}</strong><small>${t.supporting_count} supporting moment${t.supporting_count===1?'':'s'} · ${momentLinks(t,4)}${r?` · expected ${Number(r.expected_market_reaction_score||0)} · market ${Number(r.market_response_score||0)} · ${Math.max(1,Number(r.qualified_speaker_count||1))} speaker${Math.max(1,Number(r.qualified_speaker_count||1))===1?'':'s'}`:''}</small></div>`}).join('');
  const section=document.createElement('section');section.className='cx-v5-section cx-video-catalyst-detail';section.innerHTML=`<div class="cx-section-title">Creator catalysts <span class="cx-intel-context">conviction + expected reaction + independent convergence + observed market</span></div><div class="cx-v5-grid">${convictionStat}${responseStat}${gapStat}${rows}</div><small class="cx-sub">Expected reaction is deliberately conservative: it starts below Catalyst impact and only shifts toward observed creator × event × price-band outcomes as mature post-signal history accumulates. The unpriced catalyst gap is Expected reaction minus observed Market reaction. It is a prioritization lens only and never changes Scout grade or exact-SKU economics. Speaker agreement can strengthen Conviction, but multiple speakers in one video remain one independent source for Convergence.</small>`;
...
async function load(force=false){
  ...
  loading=Promise.all([
    ...
    rest('market_intel_video_opportunity_gap?select=*&order=unpriced_catalyst_gap_score.desc&limit=300').catch(()=>[])
