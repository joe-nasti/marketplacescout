...
async function load(){
  ...
  loading=Promise.all([
    ...
    rest('market_intel_entities?select=intel_id,entity_name,scryfall_id,product_id&limit=1000')
...
function matching(row){
  ...
  for(const link of links){
    const hit=(sf&&lower(link.scryfall_id)===sf)||(pid&&String(link.product_id||'')===pid)||(name&&lower(link.entity_name)===name);
    if(!hit||seen.has(link.intel_id))continue;
    const item=items.get(link.intel_id);if(item){seen.add(link.intel_id);out.push(item)}
  }
...
function decorateList(){
  ...
  document.querySelectorAll('#cxParityCards .cx-scout-card').forEach(card=>{
    ...
    const badge=document.createElement('span');badge.className='cx-intel-mini';badge.textContent=`◉ ${summary(signals)}`;badge.title='External market intelligence; does not affect Scout grade';top.appendChild(badge);
  });
}
