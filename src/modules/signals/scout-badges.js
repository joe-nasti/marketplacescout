...
function decorateDetail(sku){
  ...
  const premium=Number(c?.printing_premium_ratio||0);const printingMeta=c&&printingDemandLabel(c)?`<div class="cx-v5-component"><strong>Printing thesis · ${esc(pretty(c.printing_demand_class))}</strong><small>${esc(c.printing_demand_reason||'Printing-specific movement detected.')}</small><small>${premium>0?`Current premium vs same-finish family median ${esc((premium*100).toFixed(0))}% · `:''}${c.current_printing_release_date?`printing ${esc(c.current_printing_release_date)} · `:''}${c.newest_family_release_date?`newest family release ${esc(c.newest_family_release_date)} · `:''}${esc(c.family_printing_count??0)} known family printings</small></div>`:'';
