...
function targetForSignalEntity(el){const card=el.closest('.cx-signal-card');const id=card?.dataset?.intelId;const item=(store.get().intel?.items||[]).find(x=>String(x.intel_id)===String(id));const name=String(el.textContent||'').replace(/\s*✓\s*$/,'').replace(/^(?:Card|Format|Set|Product|Other)\s*·\s*/i,'').trim();const entity=(item?.market_intel_entities||[]).find(e=>lower(e.entity_name)===lower(name));return verifiedCardEntity(entity)?{product_id:entity.product_id,scryfall_id:entity.scryfall_id,card_name:entity.entity_name}:null}
