...
function mount(){
  ...
  wrap.innerHTML=`<summary>Source collectors</summary><div class="cx-rendered-intel-body"><p class="cx-sub">Subscribe to public RSS/Atom feeds for automated market-intelligence intake. For Discord or X content you can access but cannot automate, use the rendered-text handoff above; it enters the same canonical intake pipeline.</p><input id="cxCollectorName" placeholder="Source name (optional, e.g. MTGStocks)"><input id="cxCollectorUrl" placeholder="RSS / Atom feed URL"><div class="cx-signal-submit"><button type="button" class="cx-primary" id="cxCollectorAdd">Add feed</button><button type="button" id="cxCollectorSync">Sync now</button><span id="cxCollectorMsg" class="cx-sub"></span></div><div id="cxCollectorFeeds" class="cx-ai-result-list"></div></div>`;
