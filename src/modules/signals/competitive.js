...
async function sync(){
  ...
  if(msg)msg.textContent='Fetching and parsing up to 4 recent MTGO Challenge/Qualifier/Showcase/Trial results…';
  try{
    const session=await validSession();if(!session)throw new Error('Sign in required');
    const r=await fetch(`${collectishConfig.supabaseUrl}/functions/v1/competitive-mtgo-sync`,{method:'POST',signal:controller.signal,headers:{apikey:collectishConfig.publishableKey,Authorization:`Bearer ${session.token}`,'Content-Type':'application/json'},body:JSON.stringify({max_events:4})});
    ...
    if(msg)msg.textContent=`Imported ${data.events||0} event${Number(data.events)===1?'':'s'}, ${data.decks||0} decks and ${data.cards||0} card rows in ${Math.max(1,Math.round(Number(data.elapsed_ms||0)/1000))}s.${errorCount?` ${errorCount} event${errorCount===1?'':'s'} could not be fully imported.`:''}`;
    loading=null;await load();document.dispatchEvent(new CustomEvent('collectish:competitive-changed',{detail:data}));
  }catch(e){
    if(msg)msg.textContent=e?.name==='AbortError'?'MTGO refresh timed out after 65 seconds. Nothing will keep spinning; try again or inspect the returned event errors.':(e?.message||'Could not refresh MTGO results.');
  }finally{clearTimeout(timer);if(btn){btn.disabled=false;btn.textContent=original}}
}
