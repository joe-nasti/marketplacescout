...
async function loadMeta(force=false){
  ...
  if(!force&&Date.now()-lastMeta<60000&&meta.size)return;
  metaLoading=true;
  try{
    const rows=await rest('sealed_ev_current?select=sealed_uuid,score_components&limit=5000');
...
function removeOperationalTiles(){
  ...
  if(!detail)return;
  detail.querySelectorAll('.cx-sealed-stat').forEach(tile=>{
    const label=(tile.querySelector('span')?.textContent||'').trim().toLowerCase();
    if(label==='lifecycle'||label==='components')tile.remove();
  });
}
...
async function reconcile({forceMeta=false}={}){
  ...
  if(!root||!root.querySelector('.cx-sealed-toolbar'))return;
...
export function installSealedUiGuardian(){
  document.addEventListener('collectish:ready',()=>{
    observeSealedRoot();
    schedule({forceMeta:true});
  });
  document.addEventListener('collectish:lazy-page-loaded',e=>{
    if(e.detail?.page!=='sealed')return;
    observeSealedRoot();
    schedule({forceMeta:true});
  });
  document.addEventListener('click',e=>{
    if(e.target.closest?.('#cxSealedRefresh,[data-cx-page="sealed"]'))setTimeout(()=>schedule({forceMeta:true}),100);
    else if(e.target.closest?.('#cxSealedRows [data-deck]'))setTimeout(()=>schedule(),0);
  },true);
...
installSealedUiGuardian();

export { applyFilter as applySealedLanguageFilter, classify as classifySealedLanguagePricing };
